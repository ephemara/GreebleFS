...
- Result: added explicit cleanup coverage and max-backoff coverage for the plugin watcher fallback loop, then tightened the native watcher failure path so it serially unwinds the attempted watch registration before falling back to polling.
...
- Direct validation: inspected the self-scheduling fallback loop, the unmount cleanup branch, the capped exponential cadence, and the serial native failure unwind in `useFolderPluginRuntime`, then re-ran `bunx vitest run src/test/useFolderPluginRuntime.fallback.test.tsx` at 15:32 UTC.
- Blockers: local Vitest startup still fails because `vitest.config.ts` cannot resolve `vitest/config` or `@vitejs/plugin-react` in this environment.
...
- Tests run: `bunx vitest run src/test/useFolderPluginRuntime.fallback.test.tsx` (rechecked at 14:58 UTC).
...
- Validation: `bun test src/test/terminalCommandUtils.test.ts` (pass, revalidated this pass at 14:51 UTC). Direct inspection confirmed `FileExplorer.tsx` still exposes `Open in Terminal` for directory entries.
...
- Validation: `bun test src/test/terminalCommandUtils.test.ts` (pass, rerun at 14:56 UTC). Direct inspection also confirmed the plugin fallback unwind and `FileExplorer.tsx` still exposes `Open in Terminal` for directory entries.
...
- 2026-04-14, 14:49 UTC pass: re-ran `bun test src/test/terminalCommandUtils.test.ts`, and it passed again; direct inspection still confirms `FileExplorer.tsx` exposes `Open in Terminal` for directory entries.
