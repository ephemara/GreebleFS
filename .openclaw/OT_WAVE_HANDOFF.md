...
Last updated: 2026-04-14T19:47:00Z
...
- Direct validation: inspected the self-scheduling fallback loop, the unmount cleanup branch, the capped exponential cadence, and the native failure cleanup path in `useFolderPluginRuntime`, including the cleanup-failure branch that now starts fallback immediately, keeps unmount cleanup retryable after a failure, and closes the in-flight unmount race by retrying cleanup once if disposal lands mid-cleanup, then re-ran `bunx vitest run src/test/useFolderPluginRuntime.fallback.test.tsx` at 19:35 UTC.
- Blockers: local Vitest startup still fails because `vitest.config.ts` cannot resolve `vitest/config` or `@vitejs/plugin-react` in this environment.
...
- Tests run: `bunx vitest run src/test/useFolderPluginRuntime.fallback.test.tsx` (rechecked at 19:00 UTC).
...
- 19:00 UTC rerun confirmed the same startup blocker, so the runtime lane still has no new regression gap to reopen.
...
- Tests run: `bun test src/test/terminalCommandUtils.test.ts` (pass, rerun at 18:49 UTC confirmed the PowerShell-args, cmd, and unix path cases).
...
- Validation: `bun test src/test/terminalCommandUtils.test.ts` (pass, revalidated this pass at 18:51 UTC). Direct inspection confirmed `FileExplorer.tsx` still exposes `Open in Terminal` for directory entries.
...
- Validation: `bun test src/test/terminalCommandUtils.test.ts` (pass, rerun at 18:59 UTC). Direct inspection also confirmed the plugin fallback unwind and `FileExplorer.tsx` still exposes `Open in Terminal` for directory entries.
