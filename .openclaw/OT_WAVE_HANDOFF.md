...
Last updated: 2026-04-14T18:47:00Z
...
- Tests run: `bunx vitest run src/test/useFolderPluginRuntime.fallback.test.tsx` (rechecked at 17:59 UTC).
...
- 17:59 UTC rerun confirmed the same startup blocker, so the runtime lane still has no new regression gap to reopen.
...
- Tests run: `bun test src/test/terminalCommandUtils.test.ts` (pass, rerun at 17:49 UTC confirmed the PowerShell-args, cmd, and unix path cases).
...
- Validation: `bun test src/test/terminalCommandUtils.test.ts` (pass, revalidated this pass at 17:51 UTC). Direct inspection confirmed `FileExplorer.tsx` still exposes `Open in Terminal` for directory entries.
...
- Validation: `bun test src/test/terminalCommandUtils.test.ts` (pass, rerun at 17:58 UTC). Direct inspection also confirmed the plugin fallback unwind and `FileExplorer.tsx` still exposes `Open in Terminal` for directory entries.
