...
- Validation state: `bun test src/test/terminalCommandUtils.test.ts` passed this pass, direct inspection still confirms `FileExplorer.tsx` exposes `Open in Terminal` for directory entries, and `useFolderPluginRuntime.ts` already unwinds a failed watcher registration into bounded polling while the fallback test covers the cleanup, backoff, and unmount cases.
...
- Result: added explicit cleanup coverage and max-backoff coverage for the plugin watcher fallback loop, then tightened the native watcher failure path so it clears the attempted watch before entering bounded fallback polling, and treats failed unwatch cleanup as logged, non-fatal fallback setup noise.
...
- Direct validation: inspected the self-scheduling fallback loop, the unmount cleanup branch, the capped exponential cadence, and the native failure cleanup path in `useFolderPluginRuntime`, including the cleanup-failure branch that now stays non-fatal, then re-ran `bunx vitest run src/test/useFolderPluginRuntime.fallback.test.tsx` at 16:42 UTC.
- Blockers: local Vitest startup still fails because `vitest.config.ts` cannot resolve `vitest/config` or `@vitejs/plugin-react` in this environment.
- Next step: runtime lane should confirm the backoff cadence, native cleanup, cleanup-failure handling, and unmount cleanup once the toolchain is repaired, then stop unless it finds a concrete regression gap.
...
- Tests run: `bunx vitest run src/test/useFolderPluginRuntime.fallback.test.tsx` (rechecked at 15:58 UTC).
...
- 15:58 UTC rerun confirmed the same startup blocker, so the runtime lane still has no new regression gap to reopen.
...
- Validation: `bun test src/test/terminalCommandUtils.test.ts` (pass, revalidated this pass at 15:51 UTC). Direct inspection confirmed `FileExplorer.tsx` still exposes `Open in Terminal` for directory entries.
...
- Validation: `bun test src/test/terminalCommandUtils.test.ts` (pass, rerun at 15:56 UTC). Direct inspection also confirmed the plugin fallback unwind and `FileExplorer.tsx` still exposes `Open in Terminal` for directory entries.
