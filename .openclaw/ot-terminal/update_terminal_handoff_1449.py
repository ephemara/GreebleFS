...
text = text.replace('Last updated: 2026-04-14T14:45:00Z', 'Last updated: 2026-04-14T14:49:00Z')
...
"""
if old not in text:
    raise SystemExit('terminal handoff block not found')
text = text.replace(old, new, 1)
text = text.replace('Validation: `bun test src/test/terminalCommandUtils.test.ts` (pass, rerun at 13:55 UTC). Direct inspection also confirmed `FileExplorer.tsx` still exposes `Open in Terminal` for directory entries.', 'Validation: `bun test src/test/terminalCommandUtils.test.ts` (pass, rerun at 14:49 UTC). Direct inspection also confirmed `FileExplorer.tsx` still exposes `Open in Terminal` for directory entries.')
text = text.replace('- 2026-04-14, 13:55 UTC pass: re-ran `bun test src/test/terminalCommandUtils.test.ts`, and it passed again; direct inspection still confirms `FileExplorer.tsx` exposes `Open in Terminal` for directory entries.', '- 2026-04-14, 14:49 UTC pass: re-ran `bun test src/test/terminalCommandUtils.test.ts`, and it passed again; direct inspection still confirms `FileExplorer.tsx` exposes `Open in Terminal` for directory entries.')
wave.write_text(text, encoding='utf-8')

mem = Path(r'F:\apps-2d\overlayterm\.openclaw\ot-terminal\MEMORY.md')
text = mem.read_text(encoding='utf-8')
if '14:49 UTC rerun' not in text:
