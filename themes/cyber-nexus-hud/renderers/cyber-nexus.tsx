...
export default defineThemeRenderer({
  ...
  component({ host }) {
    ...
    // Phase to slowly rotate the background perspective grid.
    const [phase, setPhase] = useState(0);
    useEffect(() => {
      let raf: number;
      const tick = () => {
        setPhase(prev => prev + 0.001);
    ...
    return (
      <div style={{ position: 'relative', display: 'flex', flex: 1, minHeight: 0, overflow: 'hidden', perspective: '1600px', backgroundColor: '#020406' }}>
        ...
        <div style={{
          ...
          transform: `rotateX(75deg) rotateZ(${phase * 15}deg) translateY(-200px) translateZ(-600px)`,
          pointerEvents: 'none',
        }} />
        ...
        <div style={{
          position: 'absolute',
          inset: 0,
          background: 'radial-gradient(circle at 50% 50%, transparent 20%, #020406 85%)',
          pointerEvents: 'none',
        }} />
        ...
        <div style={{ position: 'relative', zIndex: 1, width: '100%', height: '100%', display: 'flex', flexDirection: 'column', padding: 24, paddingBottom: 0 }}>
           ...
           <div style={{ flex: 1, position: 'relative', transformStyle: 'preserve-3d' }}>
               {panels.map((panel, idx) => {
                  ...
                  // Add a slow orbital breathing effect for inactive panels
                  const offsetScale = Math.sin(phase + angle);
                  ...
                  return (
                     <div key={panel.id} style={{
                        ...
                        transform: isActive 
                            ...
                            : `translate(calc(-50% + ${Math.cos(angle) * 500}px), calc(-50% + ${Math.sin(angle) * 350 + offsetScale * 50}px)) scale(0.4) translateZ(-400px) rotateY(${Math.cos(angle) * 45}deg) rotateX(${Math.sin(angle) * 20}deg)`,
                        ...
                        overflow: 'hidden'
                     }} onClick={() => { if (!isActive) host.activatePanel(panel.id); }}>
