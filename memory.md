...
- Durable product note:
  ...
  - The oversized `icons-xl+` state is intentionally explorer-local. `src/components/FileExplorer.tsx` preserves that oversized live zoom after the idle commit instead of snapping straight back to the persisted `icons-xl` metrics, but external/manual view-mode changes still reset through the durable settings contract.
  ...
  - The main hot-path cleanup in `FileExplorer.tsx` now memoizes the rail, toolbar, and preview subtrees so live zoom frames stop rebuilding those panes. It also disables per-entry size transitions while the live zoom spring is active, which avoids stacking CSS transitions on top of continuous layout changes.
- Validation:
