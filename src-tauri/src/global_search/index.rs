...
pub(super) const GLOBAL_SEARCH_DIRECTORY: &str = "global-search";
const GLOBAL_SEARCH_INDEX_DIRECTORY: &str = "index";
const GLOBAL_SEARCH_STATUS_FILENAME: &str = "status.json";
pub(super) const GLOBAL_SEARCH_SCHEMA_VERSION: u32 = 1;
...
pub(super) fn global_search_data_dir(base_dir: &Path) -> PathBuf {
    base_dir.join(GLOBAL_SEARCH_DIRECTORY)
}

pub(super) fn global_search_index_dir(base_dir: &Path) -> PathBuf {
...
pub(super) fn validate_index(index_path: &Path, base_dir: &Path) -> bool {
    ...
    if meta.schema_version != GLOBAL_SEARCH_SCHEMA_VERSION || !index_path.exists() {
        return false;
    }
