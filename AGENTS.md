...
-Git COMMIT and push after recent changes. DO NOT MAKE new branches unless the user ask you to make a new one. IF the codebase is already on a branch simply remain in that one.

### If working in the frontend of any project that involve TS - ALWAYS FEEL FREE TO USE JS ALONGSIDE TS WHENEVER - WE MAKE CODE THAT RIDES LIKE AN OX- DO NOT GIVE A SHIT ABOUT HUMAN READABILITY WHATSOEVER -- HUMAN READABILITY SHOULD BE THE LEAST OF YOUR CONCERN... IF YOU NEED TO BUST OUT A COMPLEX MATHEMATICAL SIMULATION IN PARTS OF CODE TO MAKE IT MORE PERFORMANT, GO AHEAD - THIS IS RECOMMENDED. 

### Skills
If you build out a new feature or pipeline in a codebase (such as GreebleFS) at the very end of your turn use the $SkillCreator skill to explain to future agents how to work on that specific pipeline. This is important as without skills, future agents will be left on an investigation and might not be able to work on it properly. Furthermore if updating a specific a pipeline and the changes are significant, make sure to update the said skill etc.
