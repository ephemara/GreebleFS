...
import { ensureSymmetryWasm } from './WasmService';
  ...
  // Try WASM
  {
    ...
    if (positions && indices) {
      try {
        ...
        const result = ensureSymmetryWasm(posArray, idxArray, {
          tolerance,
          x: symmetryAxes.x,
          y: symmetryAxes.y,
          z: symmetryAxes.z
        ...
        if (result) {
          ...
          // Recompute normals as WASM returns empty normals for now
        ...
        console.warn("WASM symmetry failed, falling back to JS", e);
      }
    }
  }
  const posArray = positions.array as Float32Array;
