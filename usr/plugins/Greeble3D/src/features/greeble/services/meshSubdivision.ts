...
import { getGreeble3DWasmInterface } from './WasmService';

/**
 * Helper to call WASM subdivision
 */
function subdivideGeometryWasm(posArray: Float32Array, idxArray: any, params: any): any {
  const wasm = getGreeble3DWasmInterface();
  if (wasm && wasm.subdivide_geometry_wasm) {
    return wasm.subdivide_geometry_wasm(posArray, idxArray, params);
  }
  return null;
}
...
export function subdivideGeometryLocally(
  ...
  if (positions && indices) {
    try {
      ...
      // Handle both Uint16 and Uint32 arrays
      const idxArray = indices.array as any; 
      ...
      const result = subdivideGeometryWasm(posArray, idxArray, wasmParams);
      
      if (result) {
        const newGeo = new THREE.BufferGeometry();
        newGeo.setAttribute('position', new THREE.BufferAttribute(new Float32Array(result.positions), 3));
        
        // Use Uint32Array for indices if count > 65535, otherwise Uint16 could suffice but Uint32 is safe
        newGeo.setIndex(new THREE.BufferAttribute(new Uint32Array(result.indices), 1));
      ...
      console.warn("WASM subdivision failed, falling back to JS", e);
    }
  }

  // Clone the geometry to prevent concurrent modification issues
  const workingGeometry = geometry.clone();
  const workingPositions = workingGeometry.getAttribute('position');
  if (!workingPositions) return geometry;

  const posArray = workingPositions.array as Float32Array;
  const vertices: THREE.Vector3[] = [];

  // Convert positions to Vector3 array
  for (let i = 0; i < workingPositions.count; i++) {
  ...
  // Get or create index
  let workingIndices = workingGeometry.getIndex();
  let indexArray: number[];

  if (workingIndices) {
    indexArray = Array.from(workingIndices.array);
  } else {
    indexArray = [];
