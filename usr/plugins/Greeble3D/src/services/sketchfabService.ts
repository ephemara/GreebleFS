...
const SKETCHFAB_API = greeble3dRuntimeConfig.integrations.sketchfab.apiBaseUrl;

// Sort options for randomization effect
const SORT_OPTIONS = greeble3dRuntimeConfig.integrations.sketchfab.sortOptions;
...
export const searchSketchfab = async (query: string, options: SearchOptions = {}): Promise<SketchfabSearchResult> => {
    ...
    const {
        token,
        count = greeble3dRuntimeConfig.integrations.sketchfab.searchCount,
        cursor,
        randomize = false,
    } = options;
    ...
    const downloadableParam = greeble3dRuntimeConfig.integrations.sketchfab.downloadableOnly ? '&downloadable=true' : '';
    let url = `${SKETCHFAB_API}/search?type=models&q=${encodeURIComponent(query)}${downloadableParam}&sort_by=${sortBy}&count=${count}`;
    if (cursor) {
        url += `&cursor=${cursor}`;
    }
