...
async lanShareStart(path: string, shareMode: string, hubPaths: string[] | null) : Promise<Result<LanShareResult, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("lan_share_start", { path, shareMode, hubPaths }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
export type LanShareResult = { address: string; mdns_address: string | null; ios_address: string | null }
