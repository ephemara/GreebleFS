...
async ipcReplayStream(id: string, fromSequence: number | null, limit: number | null) : Promise<Result<IpcStreamReplayResponse, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("ipc_replay_stream", { id, fromSequence, limit }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
},
async ipcGetStreamStatus(id: string) : Promise<Result<IpcStreamStatus, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("ipc_get_stream_status", { id }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
export type IpcStreamReplayPacket = { metadata: IpcStreamPacketMetadata; frameMetadata: MessageRingFrameMetadata; payloadJson: string }
export type IpcStreamReplayResponse = { streamId: string; packets: IpcStreamReplayPacket[]; replayGap: MessageRingReplayGap | null; telemetry: MessageRingTelemetry }
export type IpcStreamStatus = { handle: IpcStreamHandle; telemetry: MessageRingTelemetry }
 ...
 * canonical enum.
 */
export type RuntimeKind = "native-sidecar" | "native-command" | "native-tui" | "wasm-panel" | "wasm-worker"
...
export type RuntimeToolchainStatus = { go: ToolchainProbe; tinygo: ToolchainProbe; cargo: ToolchainProbe; rustc: ToolchainProbe; wasmBindgen: ToolchainProbe; cc: ToolchainProbe; python: ToolchainProbe; manifestPath: string | null }
...
export type UsrManagedContentDirectoryStack = { laneId: string; profileMode: UsrProfileLaneMode; directories: string[]; sharedRootDirectory: string; bundledDirectory: string | null; profileDirectory: string | null; writableDirectory: string }
