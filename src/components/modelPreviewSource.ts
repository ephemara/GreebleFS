...
const MODEL_PREVIEW_TEXTURE_MIME_TYPE_BY_EXTENSION = {
  avif: 'image/avif',
  basis: 'image/x-basis',
...
async function readModelPreviewBytes(
  ...
  return readExplorerPreviewBytes(path, maxBytes).catch((error) => {
    throw error;
  });
}
...
function getPreviewBlobMimeType(path: string): string {
  ...
  return MODEL_PREVIEW_TEXTURE_MIME_TYPE_BY_EXTENSION[extension] ?? 'application/octet-stream';
}
