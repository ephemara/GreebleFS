...
import {
  ...
  getEntryTypeLabel as getSharedEntryTypeLabel,
  sortExplorerEntries as sortSharedExplorerEntries,
  type ExplorerVisibleEntriesComputeInput,
  type ExplorerVisibleEntriesSortKey,
...
import {
  ...
  humanizeExplorerChromeControlId,
...
type ExplorerSortKey = ExplorerVisibleEntriesSortKey;
...
function resolveExplorerPreviewDropTarget(
  ...
  if (
    !targetPath ||
    isCloudExplorerPath(targetPath) ||
    isExplorerVirtualPath(targetPath)
  ) {
    return null;
  }
...
type ExplorerRenderedChromeControlDefinition =
...
export type ExplorerExternalChromeControlDefinition =
...
function clampExplorerLayoutBandMetricValue(
  ...
  const bounds = EXPLORER_LAYOUT_BAND_HEIGHT_BOUNDS[metricKey];
...
function compareExplorerEntries(
  ...
  return compareSharedExplorerEntries(left, right, sortBy, sortOrder);
}
...
function sortExplorerEntries(
  ...
  return sortSharedExplorerEntries(entries, sortBy, sortOrder);
}
...
function PreviewPanel({
  ...
  const previewHeaderRowStyle = useMemo<CSSProperties>(
    () => ({
      ...
      gap: 10,
      minWidth: 0,
      minHeight: previewHeaderHeightPx,
      flexWrap: "wrap",
    }),
    [previewHeaderHeightPx],
  );
  ...
  const previewBuiltInChromeControlRegistry = useMemo<
    ExplorerRenderedChromeControlDefinition[]
  >(
    () => [
      {
  ...
  const previewChromeControlRegistry = useMemo<
    ExplorerRenderedChromeControlDefinition[]
  >(
    () => [...previewBuiltInChromeControlRegistry, ...externalChromeControls],
  ...
  const previewGlassBlurStyle: CSSProperties = {
    backdropFilter: resolveInnerSurfaceBlurFilter({
      enabled: blurEnabled && explorerTheme.previewStyle === "glass",
      blurPx: 18,
    }),
    WebkitBackdropFilter: resolveInnerSurfaceBlurFilter({
      enabled: blurEnabled && explorerTheme.previewStyle === "glass",
      blurPx: 18,
    }),
  };
          ...
          {preview.type === "image" && preview.content && (
            <ExplorerImageEditor
              ...
              cutoutBackendPreference={
                imageCutoutModelBinding.backendPreference
              }
...
export function FileExplorer({
  ...
  const {
    ...
    useShallow((state) => ({
      ...
      setFollowThemeExplorerLayout: state.setFollowThemeExplorerLayout,
  ...
  const storedActionsVisible = useExplorerStore(
    (state) =>
      state.sessions[instanceId]?.actionsVisible ??
  ...
  const [optimisticExplorerLayouts, setOptimisticExplorerLayouts] = useState<
  ...
  const themeExplorerLayoutId =
    appearance?.baseTheme.defaultExplorerLayoutId?.trim() ||
    appearance?.theme.defaultExplorerLayoutId?.trim() ||
    null;
  const canonicalExplorerLayout = useMemo(
    () =>
      findExplorerLayoutById(
  ...
  const resolvedExplorerLayout = useMemo(
    () =>
      findExplorerLayoutById(
        availableExplorerLayouts,
        activeExplorerLayoutSelectionId,
  ...
  const resolveExplorerInnerBlurFilter = useCallback(
    (blurPx: number) =>
      resolveInnerSurfaceBlurFilter({
        enabled: explorerBlurEnabled,
        blurPx,
        platform: runtimePlatform,
      }),
    [explorerBlurEnabled, runtimePlatform],
  );
  ...
  const semanticIndexModelBinding = useMemo(
    () =>
      resolveLocalModelBindingForRoot(
  ...
  const [actionsWidth, setActionsWidth] = useState(() => {
    if (typeof initialSession.actionsWidth === "number") {
      return Math.max(
        EXPLORER_ACTIONS_WIDTH_BOUNDS.min,
        Math.min(
          EXPLORER_ACTIONS_WIDTH_BOUNDS.max,
          initialSession.actionsWidth,
        ),
      );
    }
    return EXPLORER_ACTIONS_WIDTH_BOUNDS.default;
  });
  ...
  const [previewWorkflowContext, setPreviewWorkflowContext] = useState<Pick<
    ExplorerMenuPreviewContext,
    "workflowTabId" | "workflowBaseMode"
  > | null>(null);
  ...
  const [constellationActiveLens, setConstellationActiveLens] =
    useState<ConstellationLensId>(
      () => initialSession.constellation.activeLens,
    );
  ...
  const [pickerOverwriteTargetPath, setPickerOverwriteTargetPath] = useState<
    string | null
  >(null);
  ...
  const previewTerminalWorkingDirectory =
    currentPathIsCloud || currentPathIsVirtual
      ? null
      : preview.type === "text" &&
  ...
  useEffect(() => {
    if (!currentPath || currentPathIsCloud || currentPathIsVirtual) {
      return;
    }
  ...
  }, [currentPath, currentPathIsCloud, currentPathIsVirtual]);
  ...
  useEffect(() => {
    setActionsVisible(storedActionsVisible);
  }, [storedActionsVisible]);
  ...
  useEffect(() => {
    setActionsWidth((current) =>
      Math.max(
        EXPLORER_ACTIONS_WIDTH_BOUNDS.min,
        Math.min(EXPLORER_ACTIONS_WIDTH_BOUNDS.max, current),
      ),
    );
  }, []);
  ...
  const runSearch = useCallback(
    async (query: string, requestId: number) => {
      ...
      if (!supportsSearch(currentPath)) {
        ...
        setSearchLoading(false);
        if (!isExplorerVirtualPath(currentPath)) {
        ...
        if (searchMode === "semantic") {
          const response = hasSemanticSimilarityTarget
            ? await findSimilarExplorerSemantic({
                ...
                modelId:
                  semanticIndexSummary?.modelId ??
                  semanticIndexModelBinding.modelId,
                backendPreference: semanticIndexModelBinding.backendPreference,
              })
          ...
          if (isActiveSearchRequest()) {
            ...
            recordExplorerMetric({
              ...
              metadata: {
                ...
                semanticIndexedFileCount: response.diagnostics.indexedFileCount,
  ...
  useEffect(() => {
    ...
    if (
      externalRevealRequest.sequence === lastWorkspaceRevealSequenceRef.current
    ) {
      return;
    }
  ...
  const baseVisibleEntriesWorkerRequestIdRef = useRef(0);
  useEffect(() => {
    ...
    let cancelled = false;
    const requestId = baseVisibleEntriesWorkerRequestIdRef.current + 1;
    baseVisibleEntriesWorkerRequestIdRef.current = requestId;

    void computeExplorerBaseVisibleEntriesInBackground(baseVisibleEntriesInput)
      .then((nextEntries) => {
        if (
          cancelled ||
          baseVisibleEntriesWorkerRequestIdRef.current !== requestId
        ...
        startTransition(() => {
          setBackgroundBaseVisibleEntries(nextEntries);
        });
      })
      .catch((error) => {
        if (
  ...
  const homeBookmarkItems = useMemo(
    () =>
      explorerRail.nodes
        .filter(
          (
  ...
  const homeQuickAccess = useMemo(
    () => createExplorerHomeQuickAccessItems(userHomePath),
    [userHomePath],
  );
  ...
  const resolvedHomePackSelection = useMemo(
    () =>
      resolveExplorerHomePackSelection({
        ...
        themeDefaultPackId: appearance?.baseTheme.defaultHomePackId ?? null,
      }),
    [
      appearance?.baseTheme.defaultHomePackId,
      homePacks,
      homeSettings.activePackId,
    ],
  );
  const activeHomePackId = resolvedHomePackSelection.activePack?.id ?? null;
  const activeHomePackState = useMemo<Record<string, unknown>>(
    () =>
      activeHomePackId
        ? (homeSettings.packStateById[activeHomePackId] ?? {})
        : {},
    [activeHomePackId, homeSettings.packStateById],
  );
  const constellationPinnedPathSet = useMemo(
    () => new Set(constellationPinnedPaths),
    [constellationPinnedPaths],
  );
  const shouldBuildAdaptiveSemanticBands =
    effectiveExperimentalViewMode === "adaptive-semantic-grid";
  const shouldBuildConstellationCompute =
    effectiveExperimentalViewMode === "constellation";
  const experimentalSemanticBands = useMemo(
    () =>
      shouldBuildAdaptiveSemanticBands
  ...
  const constellationRouteState = useMemo(
    () =>
      shouldBuildConstellationCompute && constellationRouteModeEnabled
        ? resolveConstellationRouteState({
  ...
  const goHome = useCallback(() => {
    void navigate(EXPLORER_HOME_PATH);
  }, [navigate]);
  ...
  const explorerPickerAllowsMultiple = explorerPicker
    ...
    : true;
  const canConfirmExplorerPickerSelection = explorerPickerSelections.length > 0;
  ...
  const buildExplorerDragAvatarStackItems = useCallback(
    (
      ...
      const orderedEntries = [
        primaryEntry,
        ...dragEntries.filter(
          (candidate) => candidate.path !== primaryEntry.path,
        ),
  ...
  const executeTransferRequest = useCallback(
    async (
      ...
      if (
        request.sources.some((source) => isExplorerArchiveVirtualPath(source))
      ) {
        throw new Error(
  ...
  const refreshExplorerAfterTransferRequest = useCallback(
    async (
      request: Pick<PendingExplorerTransferRequest, "targetDir" | "sources">,
    ) => {
  ...
  useEffect(() => {
    ...
    win
      .onDragDropEvent(async (event) => {
        ...
        const payloadPaths = Array.isArray((event.payload as any).paths)
          ? ((event.payload as any).paths as string[]).filter(
              (path): path is string =>
                typeof path === "string" && path.trim().length > 0,
            )
          : [];
        ...
        if (event.payload.type === "drop") {
          const scaleFactor = await getWindowScaleFactor();
          const hoveredHit =
            event.payload.position && typeof event.payload.position === "object"
              ? resolveExplorerDropHitFromPoint(
  ...
  const openManagedPythonRepl = useCallback(async () => {
    ...
    const runtimeStatus =
      await getManagedPythonRuntimeStatus(pythonRuntimeConfig);
  ...
  const handleExplorerTerminalReportedWorkingDirectoryChange = useCallback(
    (cwd: string) => {
      ...
      if (
        lastExplorerTerminalShellReportedCwdRef.current ===
        normalizedReportedCwd
      ) {
        return;
      }
  ...
  useEffect(() => {
    ...
    if (
      lastExplorerTerminalExplorerAppliedCwdRef.current ===
  ...
  const resolvedWorkspaceLayoutMode = useMemo(() => {
    switch (workspacePaneCount) {
      case 2:
        return "split";
      case 3:
        return "triple";
  ...
  const updateExplorerLayoutBandMetric = useCallback(
    (metricKey: keyof ExplorerLayoutBandMetrics, nextValue: number) => {
      setLayoutBandMetricsDraft((current) => ({
        ...current,
        [metricKey]: clampExplorerLayoutBandMetricValue(metricKey, nextValue),
      }));
    },
    [],
  );
    ...
    async (mode: "build" | "rebuild" | "clear") => {
      ...
      try {
        await buildExplorerSemanticIndex({
          ...
          mode,
          routingMode: systemSettings.accelerationRoutingMode,
          modelId: semanticIndexModelBinding.modelId,
          backendPreference: semanticIndexModelBinding.backendPreference,
        });
        window.setTimeout(
          () => {
  ...
  const resolvePreviewContextMenuEntry = useCallback(() => {
    ...
    return {
      ...
      stem: isDirectory ? previewName : previewName.replace(/\.[^.]+$/, ""),
      isDirectory,
  ...
  const sendEntryToMobileDownload = useCallback(
    async (entry: ExplorerMenuInvocationEntry) => {
      ...
      if (!absolutePath || entry.isDirectory) {
        setError(
          "Select a regular file before sending it to the paired iPhone.",
        );
        return;
      }
      if (
        isCloudExplorerPath(absolutePath) ||
        isExplorerArchiveVirtualPath(absolutePath)
      ) {
      ...
      const activeSharePathIsFilesystemRoot =
        activeSharePath === "/" || /^[A-Za-z]:$/.test(activeSharePath ?? "");
      ...
      try {
        ...
        if (shouldRecordSuccessfulRun || !result.success) {
          recordExplorerActionRun({
            ...
            actionDirectory: action.directoryPath,
            currentLocation: runtimeContext.invocation.currentLocation,
            selectedPaths: runtimeContext.targetEntries.map(
              (entry) => entry.path,
            ),
            outputTarget,
            status: runStatus,
  ...
  const onRightClick = useCallback(
    (e: React.MouseEvent, entry: FileEntry) => {
      ...
      const actionEntries =
        entryWasSelected && selectedEntries.length > 0
          ? selectedEntries
          : [entry];
  ...
  const locationTitle = currentPathIsHome
    ? "Home"
    : crumbs.length > 0
      ? crumbs.map((crumb) => crumb.label).join(" / ")
      : currentPath || "Home";
  const locationLabel =
    (currentPathIsHome ? "Home" : crumbs[crumbs.length - 1]?.label) ??
    (() => {
  ...
  const startExplorerNativeOutDrag = useCallback(
    ...
    [
      explorerCustomizePointerActive,
      explorerDropScopeId,
      resolveExplorerEntriesNativeDragPaths,
      runtimePlatform,
    ],
  );
  ...
  const beginExplorerInternalPointerDrag = useCallback(
    (
      ...
      if (explorerCustomizePointerActive) {
        return;
      }
      startExplorerSharedDragSession({
  ...
  const buildExplorerPointerDragCandidate = useCallback(
    (args: {
      ...
      const requestedIntent =
        args.sourceEntries.some((item) =>
          isExplorerArchiveVirtualPath(item.path),
        ) || args.altKey
          ? "native-out"
          : "internal";
      const dragIntent =
  ...
  const executeExplorerDropTransfer = useCallback(
    async (
      ...
      const sharedDragSession = getExplorerSharedDragSession();
      const sources = readExplorerPathsFromDataTransfer({
        dataTransfer,
        fallbackPaths: sharedDragSession?.paths ?? [],
      });
      if (sources.length === 0) {
  ...
  useEffect(() => {
    ...
    const finalizeInternalPointerDrag = (event: PointerEvent | null) => {
      ...
      const interactionState = getExplorerDragInteractionState();
      const targetPath = interactionState.valid
        ? interactionState.targetPath
        : null;
      if (targetPath) {
        finishExplorerDragInteractionPresentation({
  ...
  const activeChromeEditSession = useMemo(
    () =>
      ...
      chromeEditSession.layoutId === effectiveChromeLayoutId
        ? chromeEditSession
        : null,
    [chromeEditSession, effectiveChromeLayoutId, explorerChromeThemeId],
  );
  const explorerChromeOverride = useMemo(
    ...
    [activeChromeEditSession, persistedExplorerChromeOverride],
  );
  const currentExplorerTabStripVisible = useMemo(() => {
    const explicitTabStripEntry =
      explorerChromeOverride?.entries.find(
        (entry) => entry.controlId === "workspaceTabStrip",
      ) ??
      persistedExplorerChromeOverride?.entries.find(
        (entry) => entry.controlId === "workspaceTabStrip",
      ) ??
      null;
    if (explicitTabStripEntry) {
      return explicitTabStripEntry.hidden !== true;
    }
    return resolvedExplorerLayout?.tabStripVisible ?? true;
  }, [
  ...
  const saveCurrentExplorerLayout = useCallback(
    async ({ duplicate }: { duplicate: boolean }) => {
      ...
      const shouldForkLayout = duplicate || baseLayout.readOnly;
      const localId = shouldForkLayout
        ? `${baseLayout.localId}-${Date.now().toString(36)}`
        : baseLayout.localId;
      const name = duplicate
        ? `${baseLayout.name} Copy`
        : baseLayout.readOnly
          ? `${baseLayout.name} Custom`
          : baseLayout.name;
      const savedLayout = await saveUserExplorerLayout(
        {
          id: localId,
          name,
          description:
            baseLayout.description ??
            "User-authored explorer layout saved from the live shell.",
          tags: [...baseLayout.tags],
          modeProfileId: effectiveModeProfile.id,
          workspaceLayoutMode: resolvedWorkspaceLayoutMode,
          tabStripVisible: currentExplorerTabStripVisible,
          tabStripSurfaceId: showsGlobalChromeControls
            ? "explorerTopbar"
            : "explorerToolbar",
          paneMetrics: {
          ...
          bandMetrics: layoutBandMetricsDraft,
          chromeSnapshot:
            activeChromeEditSession?.draftOverride ??
            explorerChromeOverride ??
            persistedExplorerChromeOverride ??
            undefined,
          basedOnLayoutId: baseLayout.id,
        },
        { basedOnLayoutId: baseLayout.id },
      );
      setOptimisticExplorerLayouts((current) => {
        const nextLayouts = current.filter(
  ...
  const saveCurrentExplorerLayoutAsUser = useCallback(() => {
    void saveCurrentExplorerLayout({ duplicate: false }).catch((error) => {
      setError(error instanceof Error ? error.message : String(error));
    });
  }, [saveCurrentExplorerLayout, setError]);
  const duplicateCurrentExplorerLayout = useCallback(() => {
    void saveCurrentExplorerLayout({ duplicate: true }).catch((error) => {
      setError(error instanceof Error ? error.message : String(error));
    });
  }, [saveCurrentExplorerLayout, setError]);
  useEffect(() => {
    if (!activeChromeEditSession) {
      if (actionsPaneAutoOpenedForCustomizeRef.current) {
        actionsPaneAutoOpenedForCustomizeRef.current = false;
  ...
  }, [actionsVisible, activeChromeEditSession]);
    ...
    () =>
      new Map(
        explorerCustomizeCatalog.map(
          (entry) => [entry.commandId, entry] as const,
        ),
      ),
  ...
  const pendingExplorerHotkeyControlId =
    chromeHotkeyCaptureControlId ??
    activeChromeEditSession?.pendingHotkeyControlId ??
    null;
  const pendingExplorerHotkeyEntry = useMemo(
    () =>
      pendingExplorerHotkeyControlId
        ? resolveExplorerCustomizeCatalogEntry(
            explorerCustomizeCatalog,
            pendingExplorerHotkeyControlId,
          )
        : null,
    [explorerCustomizeCatalog, pendingExplorerHotkeyControlId],
  );
  const pendingExplorerHotkeyPrompt = useMemo(
    () =>
      pendingExplorerHotkeyControlId
        ? `Press a key combination to bind ${
            pendingExplorerHotkeyEntry?.label ??
            humanizeExplorerChromeControlId(pendingExplorerHotkeyControlId)
          }.`
        : null,
    [pendingExplorerHotkeyControlId, pendingExplorerHotkeyEntry],
  );
  ...
  const updateExplorerChromeEditEntry = useCallback(
    (
      ...
      const fallbackSurfaceId =
        visiblePlacement?.surfaceId ?? "explorerToolbar";
  ...
  const requestExplorerChromeHotkeyCapture = useCallback(
    (controlId: ExplorerChromeControlId) => {
  ...
  const selectExplorerCustomizeControl = useCallback(
    (controlId: ExplorerChromeControlId | null) => {
      if (activeChromeEditSession) {
        setChromeEditSelectedControl(controlId);
        return;
      }
      setActionsPaneSelectedControlId(controlId);
    },
  ...
  const beginCatalogExplorerChromePointerDrag = useCallback(
    (
      ...
      beginExplorerCustomizePointerSession({
        ...
        onActivate: (dragControlId) => {
          setChromeEditDraggingControl(dragControlId);
  ...
  const explorerChromeEditMode = useMemo<ExplorerChromeEditModeState>(
    () => ({
      ...
      onRegisterSurface: activeChromeEditSession
        ? registerChromeEditSurface
        : undefined,
      onUnregisterSurface: activeChromeEditSession
        ? unregisterChromeEditSurface
        : undefined,
  ...
  const actionsPaneVisible = actionsVisible;
  ...
  const semanticIndexStatusLabel = useMemo(() => {
    if (!semanticIndexSummary || currentPathIsCloud || currentPathIsHome) {
  ...
  const handleConstellationEdgeHover = useCallback(
    (
      ...
      const fromEntry =
        ...
        constellationGraph.nodes.find(
          (node) => node.entry.path === connection.fromPath,
        )?.entry ??
        null;
      const toEntry =
        visibleEntryLookup.get(connection.toPath) ??
        constellationGraph.nodes.find(
          (node) => node.entry.path === connection.toPath,
        )?.entry ??
        null;
      ...
      setConstellationHoverState({
        ...
        reasons: getConstellationEdgeReasonsForLens(
          edge,
          constellationActiveLens,
        ),
  ...
  const handleConstellationNodeHover = useCallback(
    (
      ...
      const localEdges =
        constellationAdjacencyLookup.get(node.entry.path) ?? [];
  ...
  const actionsPaneShellStyle = useMemo<CSSProperties>(
    () => ({
      display: "flex",
      minHeight: 0,
      minWidth: 0,
  ...
  const hasDockedFileAreaPane = previewSplitIsPane || actionsPaneVisible;
  const fileAreaStyle = useMemo<CSSProperties>(
    () => ({
      ...
      overflow: "hidden",
      background: "var(--overlay-explorer-content-bg)",
      gap: hasDockedFileAreaPane ? 10 : 0,
      padding: hasDockedFileAreaPane ? "10px" : 0,
      boxSizing: "border-box",
    }),
    [hasDockedFileAreaPane],
  );
  ...
  const getExplorerTopbarRowStyle = useCallback(
    (rowId: string): CSSProperties => {
      switch (rowId) {
        case "primary":
          return explorerTopbarRowStyle;
        case "secondary":
          return explorerTopbarRowStyle;
        default:
          return explorerTopbarRowStyle;
    ...
    [explorerTopbarRowStyle],
  );
  const getExplorerToolbarRowStyle = useCallback(
    (rowId: string): CSSProperties => {
      switch (rowId) {
        case "primary":
          return explorerToolbarPrimaryRowStyle;
        case "secondary":
          return explorerToolbarSecondaryRowStyle;
        default:
          return explorerToolbarPrimaryRowStyle;
      }
    },
  ...
  const getExplorerStatusBarRowStyle = useCallback(
    (): CSSProperties => ({
      ...
      width: "100%",
      minHeight:
        layoutBandMetricsDraft.explorerStatusBarHeightPx ??
        resolvedExplorerLayout?.bandMetrics.explorerStatusBarHeightPx,
      minWidth: 0,
      flexWrap: "nowrap",
    }),
    [
  ...
  const renderExplorerActionChromeControl = useCallback(
      ...
      return (
        <button
          ...
          style={{
            ...
            minWidth,
            maxWidth:
              placement.surfaceId === "explorerStatusBar" ? 220 : undefined,
            padding,
            borderRadius: "var(--overlay-explorer-control-radius)",
            border: `1px solid ${
              isMissing
    ...
    [canExecuteExplorerChromeAction, executeExplorerChromeActionControl],
  );
  const actionChromeControlRegistry = useMemo<
    ExplorerRenderedChromeControlDefinition[]
  >(
    () =>
      explorerCustomizeCatalog
  ...
  const explorerBuiltInChromeControlRegistry = useMemo<
    ExplorerRenderedChromeControlDefinition[]
  >(
    () => [
      {
      ...
      },
      {
        ...
        render: () => (
          <button
            ...
            title="Open the Home settings surface"
      ...
      },
      {
        ...
        render: () => (
          <button
            ...
            title="Refresh the Home surface"
            style={toolbarChipButtonStyle(false)}
          >
            <RefreshCw size={11} />
            <span
      ...
      },
      {
        ...
        render: () => (
          <button
            ...
            title="Open the real machine home directory"
            style={toolbarChipButtonStyle(false)}
          >
            <FolderOpen size={11} />
            <span
      ...
      },
      {
        ...
        render: () => (
          <button
            ...
            title={
              currentPathIsCloud || currentPathIsHome
                ? currentPathIsHome
                  ? "Search is not available on the Home surface"
      ...
      },
      {
        ...
        render: (placement) => (
          <button
            ...
            aria-pressed={actionsPaneVisible}
            aria-label={
              actionsPaneVisible ? "Hide actions pane" : "Open actions pane"
            }
        ...
        render: (placement) => (
          ...
          >
            ...
            >
              ...
              <button
                type="button"
                aria-label={`Open explorer layout preset menu. Current: ${resolvedExplorerLayout?.name ?? effectiveModeProfile.label}`}
                aria-haspopup="menu"
  ...
  const explorerGlobalActionChromeControls = useMemo<
    ExplorerRenderedChromeControlDefinition[]
  >(
    () =>
      actionChromeControlRegistry.filter((entry) =>
        entry.surfaces.some((surfaceId) =>
          ["explorerTopbar", "explorerToolbar", "explorerStatusBar"].includes(
            surfaceId,
          ),
        ),
      ),
  ...
  const previewExternalChromeControls = useMemo<
    ExplorerRenderedChromeControlDefinition[]
  >(
    () =>
      actionChromeControlRegistry.filter((entry) =>
  ...
  const explorerExternalChromeControls = useMemo<
    ExplorerRenderedChromeControlDefinition[]
  >(
    () =>
      externalChromeControls.filter((entry) =>
        entry.surfaces.some((surfaceId) =>
          ["explorerTopbar", "explorerToolbar", "explorerStatusBar"].includes(
            surfaceId,
          ),
  ...
  const resolveExplorerChromeCommandSurface = useCallback(
    (catalogEntry: ExplorerCustomizeCatalogEntry): ExplorerChromeSurfaceId => {
      const visiblePlacement = findRegisteredExplorerChromePlacement(
      ...
      if (
        selectedExplorerCustomizePlacement?.controlId === catalogEntry.controlId
      ) {
      ...
      if (catalogEntry.surfaces.includes("workspaceHeader")) {
        return showsGlobalChromeControls ? "explorerTopbar" : "explorerToolbar";
      }
  ...
  const activateExplorerChromeCommand = useCallback(
    (controlId: ExplorerChromeControlId): boolean => {
      const controlEntry = explorerChromeControlRegistryById.get(controlId);
      if (controlEntry?.activate) {
        controlEntry.activate();
        return true;
      }
      switch (controlId) {
        case "navigateBack":
  ...
  const statusBarStyle = useMemo<CSSProperties>(
    ...
    [explorerTheme.statusBarStyle, resolveExplorerInnerBlurFilter],
  );
  ...
  const applyLayoutZoomPointerAnchor = useCallback(
    (nextState: ExplorerLayoutZoomState) => {
      ...
      if (resolvedState.family === "grid") {
        ...
        const contentHeight =
          totalRows * rowHeight +
          Math.max(0, totalRows - 1) * nextGridMetrics.gap;
      ...
      setExplorerViewportScrollTop(
        nextScrollTop +
          (newItem.visible ? (nextRowMetrics?.newItemHeight ?? rowHeight) : 0),
      );
    },
    [
      explorerTheme,
  ...
  const renderSearchMetadata = useCallback(
    (entry: FileEntry) => {
  ...
  const getSearchTooltip = useCallback(
    (entry: FileEntry) => {
      if (!isSearchActive || !isExplorerSearchResultEntry(entry)) {
        return undefined;
      }
  ...
  const gridVirtualizedEntryElements = useMemo(() => {
    ...
    return virtualizedEntries.map((entry) => {
      ...
      return (
        ...
        >
          ...
          <div
            style={{
              width: "var(--overlay-explorer-grid-icon-stage-size)",
              height: "var(--overlay-explorer-grid-icon-stage-size)",
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
              overflow: "hidden",
          ...
          {isRenaming ? (
            ...
            <span
              style={{
                ...
                textAlign:
                  explorerTheme.labelMode === "inline" ? "left" : "center",
                color: EXP.text,
                overflow: "hidden",
                textOverflow: "ellipsis",
                display: "-webkit-box",
          ...
          <span
            style={{
              fontSize: "var(--overlay-explorer-entry-meta-size)",
              textAlign:
                explorerTheme.labelMode === "inline" ? "left" : "center",
              color: EXP.muted2,
              overflow: "hidden",
    ...
    activeGridMetrics,
    appearance,
    bindExplorerEntryMotion,
    cancelRename,
    commitRename,
    dropEntrySurface,
    explorerGridStageSizeTransition,
    explorerTheme.labelMode,
    getExplorerDirectoryDropBinding,
    getExplorerEntryDragPresentation,
    getExplorerEntryIconSrc,
      ...
      return (
        ...
        >
          ...
          >
            <div
              style={{
                width: "var(--overlay-explorer-list-thumbnail-size)",
                height: "var(--overlay-explorer-list-thumbnail-size)",
                minWidth: "var(--overlay-explorer-list-thumbnail-size)",
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                overflow: "hidden",
            ...
            >
              <ExplorerEntryThumbnailStageContent
                entry={entry}
                fallbackIconSize={activeRowMetrics.iconSize ?? 16}
                fallbackIconSrc={iconSrc}
                hoverScrubEnabled={hoveredVideoThumbnailPath === entry.path}
                iconTheme={themeIconTheme}
                motionAppearance={appearance}
    ...
    return virtualizedEntries.map((entry) => {
      ...
      return (
        ...
        >
          <td
            style={{
              padding: isDetailsMode ? "6px 12px" : "4px 12px",
              verticalAlign: "top",
              overflow: "hidden",
            }}
          >
  ...
  useEffect(() => {
    const h = (e: KeyboardEvent) => {
      ...
      if (pendingHotkeyControlId) {
        ...
        if (activeChromeEditSession) {
          ...
          setActionsPaneSelectedControlId(pendingHotkeyControlId);
        }
        return;
      }
      if (
    ...
    setActionsPaneSelectedControlId,
    setCommandKeybinding,
    setChromeEditPendingHotkeyControl,
    setChromeEditSelectedControl,
    setChromeHotkeyCaptureControl,
      ...
      return (
        <div
          ...
          style={{
            ...
            boxShadow:
              dragPresentation.boxShadow ??
              semanticTableRestingSurface.boxShadow,
  ...
  const renderConstellationNode = (
    ...
    const isRouteAnchor =
      constellationRouteState.anchorPath === node.entry.path;
    ...
    const dimForRoute =
      constellationRouteActive && !isRouteAnchor && !isRouteTarget && !isSel;
  ...
  const renderConstellationField = () => {
    ...
    const constellationHudBackdropFilter = resolveExplorerInnerBlurFilter(16);
    const constellationBandChipBackdropFilter =
      resolveExplorerInnerBlurFilter(12);
    const hudCardStyle: CSSProperties = {
      borderRadius: 999,
      border: "1px solid rgba(255,255,255,0.12)",
      background: "rgba(7, 10, 16, 0.68)",
      backdropFilter: constellationHudBackdropFilter,
      WebkitBackdropFilter: constellationHudBackdropFilter,
          ...
          >
            ...
            >
              ...
              >
                ...
                <span style={{ fontSize: 11, color: EXP.muted2 }}>•</span>
                <span style={{ fontSize: 12, color: EXP.muted }}>
                  Pins {constellationVisiblePinnedCount}/
                  {constellationPinnedPaths.length}
                </span>
              </div>
              <div
                style={{
            ...
            {constellationHoverState && (
              ...
              >
                ...
                >
                  ...
                  >
                    {constellationHoverState.kind === "edge"
                      ? "Edge Reason"
                      : "Node Context"}
                  </div>
                  <div
                  ...
                  >
                    {constellationHoverState.reasons.length > 0 ? (
                      ...
                      >
                        No strong{" "}
                        {constellationLensDefinition.label.toLowerCase()} link
                        is active for this node yet.
                      </div>
                    )}
                  </div>
                </div>
            ...
            {constellationFieldLayout.bands.map((band) => (
              <div
                ...
                style={{
                  ...
                  backdropFilter: constellationBandChipBackdropFilter,
                  WebkitBackdropFilter: constellationBandChipBackdropFilter,
                  padding: "8px 12px",
  ...
  const explorerPreviewPane = useMemo(() => {
    ...
    return (
      <PreviewPanel
        ...
        previewHeaderHeightPx={
          layoutBandMetricsDraft.previewHeaderHeightPx ??
          resolvedExplorerLayout?.bandMetrics.previewHeaderHeightPx
        }
        showHiddenFiles={showHidden}
        editorSettings={editorSettings}
        shaderPerformanceMode={shaderPerformanceMode}
  ...
  const explorerActionsPane = useMemo(() => {
    ...
    return (
      ...
      >
        <ResizablePane
          size={actionsWidth}
          minSize={EXPLORER_ACTIONS_WIDTH_BOUNDS.min}
          maxSize={EXPLORER_ACTIONS_WIDTH_BOUNDS.max}
          onSizeChange={setActionsWidth}
          borderColor={`${accent}55`}
          handleSide={previewPlacement === "leading" ? "right" : "left"}
          style={actionsPaneShellStyle}
        >
          <ExplorerActionsPane
            accent={accent}
            appearance={appearance}
            ...
            customizeMode={Boolean(activeChromeEditSession)}
  ...
  return (
    ...
    >
        ...
        <div style={explorerContentRowStyle}>
          ...
          <div data-overlay-explorer-plane="main" style={mainColumnStyle}>
            ...
            >
              ...
              >
                ...
                >
                  <div
                    ...
                    data-explorer-customize-remove-zone={
                      activeChromeEditSession ? "true" : undefined
                    }
                    tabIndex={0}
              ...
              {previewPlacement === "trailing" ? explorerPreviewPane : null}
              {previewPlacement === "trailing" ? explorerActionsPane : null}
            </div>
          </div>
        </div>
      </div>
      ...
      {pendingExplorerHotkeyPrompt && !actionsPaneVisible ? (
        <div
          style={{
            position: "absolute",
            top: shouldRenderStatusBar ? 56 : 16,
            right: 16,
            zIndex: 25,
            maxWidth: 320,
            borderRadius: 12,
            border: `1px solid ${accent}66`,
            background: `color-mix(in srgb, ${accent} 12%, var(--overlay-explorer-toolbar-bg))`,
            boxShadow: "var(--overlay-explorer-popup-shadow)",
            padding: "10px 12px",
            color: EXP.text,
            fontSize: 11,
        ...
        >
          <div
            style={{
              ...
              backdropFilter: resolveExplorerInnerBlurFilter(18),
              WebkitBackdropFilter: resolveExplorerInnerBlurFilter(18),
              ...BOUNDED_CHROME_CONTAINMENT_STYLE,
            }}
          >
            <div
