...
export function ExplorerAudioWorkbench({
  ...
  const [analysis, setAnalysis] = useState<ExplorerAudioPreviewAnalysis | null>(
    null,
  );
  const [previewSource, setPreviewSource] =
    useState<ExplorerAudioPreviewSource | null>(null);
  const [playbackSource, setPlaybackSource] = useState(audioSource);
  const [playbackMimeType, setPlaybackMimeType] = useState<string | null>(
    audioMimeType,
  );
  ...
  const [convertFormat, setConvertFormat] = useState<
    "mp3" | "wav" | "flac" | "ogg"
  >(DEFAULT_EXPLORER_AUDIO_EXPORT_FORMAT_ID);
  const [exportDialogMode, setExportDialogMode] =
    useState<ExportDialogMode>(null);
  const [exportPathInput, setExportPathInput] = useState("");
  const [exportState, setExportState] = useState<ExportState>("idle");
  const [exportMessage, setExportMessage] = useState(
    "Waveform, trim, and transform controls stay inside the explorer.",
  );
  const [spectrogramSource, setSpectrogramSource] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    async function loadAudioState() {
      ...
      try {
        ...
        setPlaybackStatus(
          "Ready to scrub, trim, normalize, convert, and export.",
        );
      } catch (error) {
        if (cancelled) {
          return;
  ...
  const effectiveDuration =
    duration > 0 ? duration : (analysis?.durationSeconds ?? 0);
  const selectionDuration = Math.max(0, selectionEnd - selectionStart);
  const selectionLeft =
    effectiveDuration > 0
      ? `${(selectionStart / effectiveDuration) * 100}%`
      : "0%";
  const selectionWidth =
    effectiveDuration > 0
      ? `${(selectionDuration / effectiveDuration) * 100}%`
      : "0%";
  const playheadLeft =
    effectiveDuration > 0
  ...
  const analysisCards = useMemo(
    () => [
      ...
      },
      {
        label: "Peak",
        value: formatDb(
          analysis?.peakLevel ? 20 * Math.log10(analysis.peakLevel) : null,
        ),
      },
  ...
  function updateSelection(nextStart: number, nextEnd: number) {
    ...
    const finalEnd = clamp(
      ...
      Math.min(
        effectiveDuration || 0,
        boundedStart + MINIMUM_SELECTION_SECONDS,
      ),
      effectiveDuration || 0,
    );
  ...
  function openExportDialog(mode: Exclude<ExportDialogMode, null>) {
    ...
    const extension =
      mode === "convert"
        ? (getExplorerAudioExportFormatDefinition(convertFormat)?.extension ??
          convertFormat)
        : DEFAULT_EXPLORER_AUDIO_EXPORT_FORMAT_ID;
  ...
  return (
    <>
            ...
            >
              ...
              >
                ...
                >
                  <button
                    ...
                    onClick={() => syncCurrentTime(selectionStart)}
                  >
                    <RotateCcw size={14} />
                    Jump To In
                  </button>
              ...
              <audio
                ...
                onLoadedMetadata={(event) => {
                  const nextDuration = Number.isFinite(
                ...
                src={playbackSource}
                onError={(event) => {
                  const nextError = getAudioPlaybackErrorLabel(
                    event.currentTarget.error,
                  );
                  setPlaybackError(nextError);
                  setPlaybackStatus(
                    "Direct preview failed. Falling back to preview proxy…",
                  );
                  void ensurePlaybackProxy(true);
                }}
                style={{ width: "100%" }}
              >
                This audio preview is not supported by the current desktop
                webview.
              </audio>
            ...
            >
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    <input
                      ...
                      value={fadeInSeconds}
                      onChange={(event) =>
                        setFadeInSeconds(Number(event.target.value) || 0)
                      }
                      style={inputStyle}
                    />
                  </label>
                  ...
                  >
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        setFadeOutSeconds(Number(event.target.value) || 0)
                      }
                      style={inputStyle}
                    />
                  </label>
                  ...
                  >
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        setConvertFormat(
                          event.target.value as "mp3" | "wav" | "flac" | "ogg",
                        )
                      }
                      style={inputStyle}
                    >
                ...
                >
                  <input
                    ...
                    onChange={(event) =>
                      setGenerateSpectrogram(event.target.checked)
                    }
                  />
              ...
              >
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  ...
                  {isGeneratingProxy ? (
                    <span style={badgeStyle}>Generating Proxy…</span>
                  ) : null}
                  {isAnalyzing ? (
                    <span style={badgeStyle}>Analyzing…</span>
                  ) : null}
                </div>
                {spectrogramSource ? (
                  ...
                  <div
                    style={{ fontSize: 11, color: "var(--overlay-text-muted)" }}
                  >
                    Export with spectrogram enabled to render a frequency
                    heatmap in the preview pane.
                  </div>
                )}
              </div>
            </div>
      ...
      >
        ...
        >
          The default flow is non-destructive export. Use this only when you
          want the selected file replaced.
        </div>
      </AppConfirmDialog>
    </>
  );
