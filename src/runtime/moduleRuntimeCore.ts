...
export async function transpileRuntimeModuleGraphLocal(args: {
  ...
  const transpileModuleSource = args.transpileModuleSource ?? transpileRuntimeModuleSourceLocal;
  ...
  const compileModule = async (modulePath: string, source: string): Promise<void> => {
    ...
    const compilePromise = (async () => {
      const transpiledCode = await transpileModuleSource(source, args.prependCode ?? '');
