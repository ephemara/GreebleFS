...
export const accelerationRoutingModeOptions: readonly AccelerationRoutingModeOption[] = [
  {
    id: 'auto',
    label: 'Auto',
    description: 'Pick the best ready provider for each workload based on its default acceleration order.',
  },
  {
    id: 'preferNative',
    label: 'Prefer Native',
    description: 'Bias toward the built-in native `wgpu` lane when it can handle the workload.',
  },
  {
    id: 'preferCuda',
    label: 'Prefer CUDA',
    description: 'Bias toward the Python-sidecar CUDA/AI lane for search, inference, and media-heavy work.',
  },
  {
    id: 'cpuOnly',
    label: 'CPU Only',
    description: 'Disable accelerator routing and force the CPU fallback provider for every workload.',
  },
] as const;
...
export const accelerationWorkloadCatalog: readonly AccelerationWorkloadDefinition[] = [
  ...
  },
  {
    ...
    description: 'Decode, transcode, proxy generation, and other media-adjacent transforms.',
    defaultProviderOrder: ['cudaPython', 'wgpu', 'cpu'],
  },
  {
    id: 'highVolumePreviews',
    label: 'High-Volume Previews',
    description: 'Preview surfaces that churn through many images, frames, or waveform-style reductions.',
    defaultProviderOrder: ['wgpu', 'cudaPython', 'cpu'],
  },
  {
    id: 'aiIndexing',
    label: 'AI Indexing',
    description: 'Embedding generation and local corpus indexing for semantic search.',
  ...
  },
  {
    ...
    description: 'Vector lookup, nearest-neighbor search, and perceptual comparison lanes.',
    defaultProviderOrder: ['cudaPython', 'cpu'],
  },
  {
    id: 'directStorage',
    label: 'Direct Storage',
    description: 'Storage-to-accelerator data paths when the platform and provider support them.',
    defaultProviderOrder: ['cudaPython', 'cpu'],
  },
  {
    id: 'fileHashing',
    label: 'File Hashing',
    description: 'Bulk hashing or hash-adjacent file processing when a provider is suitable.',
    defaultProviderOrder: ['cudaPython', 'cpu'],
  },
] as const;

export function normalizeAccelerationRoutingMode(value: unknown): AccelerationRoutingMode {
  return value === 'preferNative' || value === 'preferCuda' || value === 'cpuOnly'
    ? value
    : 'auto';
}

export function getAccelerationRoutingModeLabel(value: AccelerationRoutingMode): string {
  return accelerationRoutingModeOptions.find(option => option.id === value)?.label ?? 'Auto';
}

export function getAccelerationWorkloadDefinition(
  workloadId: AccelerationWorkloadId,
): AccelerationWorkloadDefinition {
  return accelerationWorkloadCatalog.find(definition => definition.id === workloadId)
...
export function findAccelerationProviderStatus(
  ...
  return snapshot.providers.find(provider => provider.providerKind === providerKind) ?? null;
}
...
function reorderProviderOrder(
  ...
  const promote = mode === 'preferNative'
    ? 'wgpu'
    : mode === 'preferCuda'
      ? 'cudaPython'
      : null;
  ...
  return [promote, ...nextOrder.filter(providerKind => providerKind !== promote)];
}
...
export function resolveAccelerationProviderForWorkload(
  ...
  const orderedProviders = reorderProviderOrder(definition.defaultProviderOrder, mode);
  const candidateProviders = orderedProviders
    .map(providerKind => findAccelerationProviderStatus(snapshot, providerKind))
    .filter((provider): provider is AccelerationProviderStatusLike =>
      provider != null
      && provider.supportedWorkloadIds.includes(workloadId));

  const readyProvider = candidateProviders.find(provider => provider.ready)
