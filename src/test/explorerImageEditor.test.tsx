...
describe('ExplorerImageEditor', () => {
  ...
  it('uses the shared runtime editor for preview mode without showing edit chrome', async () => {
    ...
    render(
      <ExplorerImageEditor
        ...
        imageSource="asset://localhost/preview.png"
        mode="preview"
      />,
    );
    ...
    expect(screen.getByText('Interactive image preview')).toBeInTheDocument();
  ...
  it('saves edited bytes through the explorer write path and the imperative ref', async () => {
    const harness = createMockEditorHarness();
    const onSaved = vi.fn();
    ...
    await waitFor(() => {
      expect(harness.editor.shapeManager.add).toHaveBeenCalledWith({
