...
describe('ExplorerAudioWorkbench', () => {
  ...
  it('falls back to a SoX proxy when direct playback fails', async () => {
    ...
    await waitFor(() => {
      expect(createExplorerAudioPreviewProxyMock).toHaveBeenCalledWith('/tmp/anthem.mp3');
    });
  ...
  it('keeps overwrite-original behind an explicit confirmation dialog', async () => {
    ...
    await screen.findByText(/ready to scrub, trim, normalize, convert, and export/i);
    fireEvent.click(screen.getByRole('button', { name: /overwrite original/i }));

    const dialog = await screen.findByRole('dialog');
    fireEvent.click(within(dialog).getByRole('button', { name: /^overwrite original$/i }));

    await waitFor(() => {
      expect(exportExplorerAudioTransformMock).toHaveBeenCalledWith(expect.objectContaining({
        inputPath: '/tmp/anthem.mp3',
        outputPath: '/tmp/anthem.mp3',
