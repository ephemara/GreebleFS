...
describe("explorerStore persistence", () => {
  it("starts with the default explorer session snapshot", () => {
    ...
    expect(
      useExplorerStore.getState().sessions[PRIMARY_EXPLORER_INSTANCE_ID],
    ).toEqual(defaultExplorerSession);
    expect(useExplorerStore.getState().workspace).toEqual(
      defaultExplorerWorkspace,
    );
    expect(useExplorerStore.getState().rail).toEqual(
      createDefaultExplorerRailSnapshot(),
    );
  ...
  it("persists session and rail updates together", () => {
    const folderResult = createExplorerBookmarkFolder(
      createDefaultExplorerRailSnapshot(),
      { name: "Work" },
    );
    ...
    expect(hydrated.sessions[PRIMARY_EXPLORER_INSTANCE_ID]?.currentPath).toBe(
      "M:\\OverlayTerm\\src",
    );
  ...
  it("migrates legacy bookmark payloads into the new rail snapshot", () => {
    ...
    window.localStorage.setItem(
      EXPLORER_LEGACY_BOOKMARKS_KEY,
      JSON.stringify([{ name: "OverlayTerm", path: "M:\\OverlayTerm" }]),
    );
  ...
  it("restores the rail from backup through the store action", () => {
    ...
    const initialPayload = window.localStorage.getItem(
      EXPLORER_STATE_STORAGE_KEY,
    );
    window.localStorage.setItem(
      EXPLORER_STATE_BACKUP_KEY,
    ...
    expect(useExplorerStore.getState().persistence.status).toBe(
      "restored-backup",
    );
  });
    ...
    persistExplorerState(
      {
        ...
        sessions: {
          [PRIMARY_EXPLORER_INSTANCE_ID]:
            legacyFocusSession as typeof defaultExplorerSession,
        },
  ...
  it("migrates legacy searchIncludeContent into the persisted searchMode field", () => {
    const { searchMode: _searchMode, ...legacySessionBase } =
      defaultExplorerSession;
    ...
    persistExplorerState(
      legacyPersistedState as unknown as Parameters<
        typeof persistExplorerState
      >[0],
      window.localStorage,
    );

    const hydrated = loadExplorerPersistedState(window.localStorage);
    expect(hydrated.sessions[PRIMARY_EXPLORER_INSTANCE_ID]?.searchMode).toBe(
      "content",
    );
    ...
    expect(reparsed.sessions?.[PRIMARY_EXPLORER_INSTANCE_ID]?.searchMode).toBe(
      "content",
    );
    expect(
      reparsed.sessions?.[secondaryInstanceId]?.searchIncludeContent,
    ).toBeUndefined();
  ...
  it("migrates legacy pane-local tabs into workspace-owned tabs", () => {
    ...
    expect(hydrated.workspace.activeWorkspaceTabId).toBe(
      PRIMARY_EXPLORER_TAB_ID,
    );
  ...
  it("creates, focuses, and closes explorer workspace tabs", () => {
    ...
    expect(useExplorerStore.getState().workspace.activeWorkspaceTabId).toBe(
      nextTab.id,
    );
  ...
  it("preserves hidden pane sessions when moving 4-Up to 3-Up and back", () => {
    ...
    expect(tripleWorkspaceTab?.panes["pane-3"]?.instanceId).toBe(
      paneThreeInstanceId,
    );
    expect(tripleWorkspaceTab?.panes["pane-4"]?.instanceId).toBe(
      paneFourInstanceId,
    );
    ...
    expect(restoredWorkspaceTab?.panes["pane-3"]?.instanceId).toBe(
      paneThreeInstanceId,
    );
    expect(restoredWorkspaceTab?.panes["pane-4"]?.instanceId).toBe(
      paneFourInstanceId,
    );
  });
  ...
  it("preserves hidden pane sessions when moving 3-Up to 2-Up and back", () => {
    ...
    expect(splitWorkspaceTab?.panes["pane-2"]?.instanceId).toBe(
      paneTwoInstanceId,
    );
    expect(splitWorkspaceTab?.panes["pane-3"]?.instanceId).toBe(
      paneThreeInstanceId,
    );
    ...
    expect(restoredWorkspaceTab?.panes["pane-2"]?.instanceId).toBe(
      paneTwoInstanceId,
    );
    expect(restoredWorkspaceTab?.panes["pane-3"]?.instanceId).toBe(
      paneThreeInstanceId,
  ...
  it("creates a new single-pane workspace tab from the focused pane in a multi-pane workspace", () => {
    ...
    const createdWorkspaceTab = useExplorerStore
      .getState()
      .workspace.tabs.find((tab) => tab.id === nextWorkspaceTab.id);
    ...
    expect(
      useExplorerStore.getState().sessions[createdPane?.instanceId ?? ""],
    ).toMatchObject({
      currentPath: "C:\\focused-pane",
      history: ["C:\\", "C:\\focused-pane"],
      historyIdx: 1,
...
describe("explorerStore transient explorer state", () => {
  it("stores trimmed pending explorer open requests with monotonic sequence ids", () => {
    ...
    expect(useExplorerStore.getState().pendingOpenRequest).toEqual(
      firstRequest,
    );
    ...
    expect(secondRequest?.sequence ?? 0).toBeGreaterThan(
