...
- `src/components/ExplorerImageCutoutSurface.tsx`
  Shell-owned cutout lane for explorer images. It opens one native cutout session to bootstrap the current workflow mask, then switches to local mask editing for responsiveness: prompt clicks or auto-remove bootstrap the matte, local brush/erase refinement reshapes the mask, and the marching-ants overlay is drawn from the actual resolved selection boundary instead of the full image rectangle. The lane stays preview-first inside the pane: the subject stage remains fullscreen-first, the top-right action bar owns undo/redo/reset/refine/save/copy as icon-first chrome, and edge-refine sliders only surface behind an explicit `Refine` inspector instead of permanently consuming pane width or adding a tutorial card. Copy/drag/save still route through the host by sending the locally refined mask back only when output is requested.
