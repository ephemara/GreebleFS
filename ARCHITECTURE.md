...
- `src/config/hotkeys.ts`
  Shipped hotkey-catalog loader over `usr/hotkeys/**/hotkeys.json`. The default binding map is now authored data; TypeScript should stay the normalization/matching layer instead of the place where new built-in shortcut rows are authored. Global app zoom keyboard shortcuts (`appZoomIn`, `appZoomOut`) also route through this catalog, and local surfaces that must keep `Ctrl+=` / `Ctrl+-` ownership should mark a root with `data-gfs-local-app-zoom-hotkeys="true"` so shell zoom yields cleanly.
...
- Managed content roots now split by runtime mode around a canonical `usr/` backbone:
  ...
  - `usr/profiles/catalog.json`, `usr/profiles/shared/settings.json`, and `usr/profiles/<profile-id>/{profile.json,settings.json}` are the runtime-owned profile overlay contract. V1 only moves workbench config behind profiles; notes, screenshots, explorer workspace/session blobs, and other transient local storage stay global outside this system.
