...
- Universal polyglot runtime pipeline (`runtime-host-v1`):
  ...
  - `src/components/GoPanelHost.tsx` is now a compatibility wrapper over `WasmPanelHost.tsx` for the existing Go PTY / sample-panel consumers, while `src/components/PluginWasmRuntimeSurfaces.tsx` is the reusable plugin-facing wrapper for package panels and preview lanes
