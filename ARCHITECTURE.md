...
- Universal polyglot runtime pipeline (`runtime-host-v1`):
  ...
  - the Go workspace at `src-go/` owns the first-party SDK (`sdk/greeblefs-go/{ipc,runtime,hostapi,panel}`) plus reference builtin runtimes (`echo-sidecar`, `echo-command`, `sample-panel`). `sample-panel` is now the canonical interactive Go/Wasm smoke runtime: it persists local state through the host bridge, subscribes to `selection.changed` / `preview.session.changed` / `cwd.changed`, and is mounted by the built-in shell panel `go-sample-panel`. Managed-content Go packages stay outside this workspace and own their own `go.mod`
