...
- Explorer drag/drop is DOM-first by design. `src-tauri/tauri.conf.json` now sets `dragDropEnabled: false` because Tauri's internal webview file-drop system conflicts with HTML5 draggable surfaces and can make explorer-internal drags feel randomly dead or invalid. If drag/drop regresses, inspect `src/components/explorer/explorerDragAndDrop.ts` and DOM payload parsing before re-enabling Tauri's drag-drop ownership.
