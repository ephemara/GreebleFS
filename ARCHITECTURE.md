...
- `MCP/greeblefs-dev-mcp/`
  Standalone dev automation subsystem for agents and MCP clients. `src/index.ts` is the public MCP server, `src/smoke.ts` is the runtime proof harness, `src/runtime/greeblefsAutomationRuntime.ts` owns attach/status/workspace automation plus startup-phase classification, and `src/runtime/windowsDesktopWindowCapture.ts` is the Windows-native fallback for enumerating/capturing real desktop `greeblefs.exe` windows when WebView/CDP attach is unavailable or the app has not created its first WebView yet. The package uses Node + `tsx` for runtime execution because Playwright/CDP attachment is reliable there on this Windows host, while Bun remains fine for install and TypeScript validation.
...
- `src-tauri/src/preview_streaming.rs` and `src-tauri/src/archive_ops.rs`
  Native bounded preview streaming lives here. `preview_streaming.rs` owns the manifest-backed `previewStreaming` policy, chunked local preview bytes, strict UTF-8 text reads, data-URI reads, byte-limit enforcement, and cancellation checkpoints. `archive_ops.rs` exposes archive-entry preview reads for ZIP, TAR, TAR.GZ, TAR.BZ2, TAR.XZ, 7z, Gzip, Bzip2, and XZ without materializing the entry just to preview it. Public preview commands still keep existing names where possible, but local preview bytes/text/data-URI reads enter the `previewRead` graph lane, archive listing/materialization/extraction enters the `archive` lane, and large binary preview payloads stay on raw IPC responses rather than message-ring retention.
...
- `src/runtime/tauriClient.ts` and `src/runtime/explorerBackend.ts`
  Typed frontend bridge for native explorer/media commands. Large preview/artifact reads now use tauron transport binary commands (`invokeBinary`) so `fs_read_preview_bytes`, archive preview reads, cloud/remote preview reads, and runtime artifact reads return `Uint8Array` payloads without custom invoke plumbing. Explorer `Open With` association lookup, Windows shell context-menu import/invoke, and explicit app launch also belong here through the `open_with_*` commands; React should not reintroduce raw platform pickers, registry scraping, or per-component shelling-out. Explorer thumbnail callers should enter through this seam too: it now exposes both the legacy data-URL thumbnail command and the identity-aware thumbnail-artifact command used by the shared explorer thumbnail runtime.
- `src/runtime/ipc/` and `src-tauri/src/ipc_runtime/`
  Shared desktop transport adapters. `control` still stays on Specta-generated commands/events for small typed payloads, but stream/resource/binary transport primitives now come from tauron transport instead of app-owned event-name or file-path contracts. `src/runtime/ipc/` is now the app-semantic adapter layer around tauron `subscribeStream(...)`, `resourceUrl(...)`, and `Resource.close()`, while `src-tauri/src/ipc_runtime/` primarily owns artifact registration/release and app-specific file lifecycle. New high-throughput or long-lived host-facing work should extend tauron transport first, then keep only app semantics in GreebleFS instead of inventing parallel replay/resource systems.
...
- Lookdev is now a first-class semantic shell authoring lane instead of a pile of unrelated tweak surfaces:
  ...
  - `src/components/lookdev/LookdevOverlay.tsx` is the immersive in-app authoring shell. It uses `tweakpane` for the compact inspector, edits a reversible draft session over the live shell, and surfaces Save / runtime Apply / theme-export flows without becoming raw HTML/CSS page-builder truth.
...
- Universal polyglot runtime pipeline (`runtime-host-v1`):
  ...
  - every runtime is a self-describing folder containing a single `runtime.toml` (id, kind, compiler, module dir, entry, watch globs, env, args, working directory, permissions, optional `panel` / `command` / `sidecar` / `tui` blocks). Compilers are `go-native`, `go-js-wasm`, `tinygo-wasm`, `cargo-wasm-bindgen`, or `python-sidecar`
...
- `FileExplorer.tsx` keeps the control plane shared across those experimental runtimes:
  ...
  - the status-bar `statusViewToggles` cluster swaps its second button between standard zoom density and experimental granularity labels without changing the control layout
...
- Preview/archive byte-read tuning belongs in `previewStreaming` and `nativeTaskGraph.laneConcurrency.previewRead/archive` under `usr/profiles/default/explorer-performance/**/explorer-performance.json`. Large preview payloads should return over raw IPC, while message rings should carry progress/status only; do not retain image/text/archive preview bytes in host-event or IPC stream rings.
...
- The dev-session truth is intentionally split:
  ...
  - If startup looks wrong, compare both files before changing app code.
