...
- `src/components/ExplorerImageCutoutSurface.tsx`
  Shell-owned cutout lane for explorer images. It opens one native cutout session to bootstrap the initial subject mask, then switches to local mask editing for responsiveness: `Spark` grabs connected color islands, `Sweep` brushes across similar pixels, `Soft Edge` and `Edge Pull` reshape the matte locally, and the marching-ants overlay is drawn from the actual resolved selection boundary instead of the full image rectangle. The lane mirrors shared image-filter state and owns lazy drag/copy/save affordances for transparent PNG output by sending the locally refined mask back to the host only when export/clipboard/drag is requested.
