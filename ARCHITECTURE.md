...
- On this Windows 10 26100 machine, targeted `cargo test --manifest-path src-tauri/Cargo.toml ... --lib` still compiles but the produced lib-test executable aborts before `main` with `STATUS_ENTRYPOINT_NOT_FOUND` / `0xc0000139`. `scripts/validate-runtime-stack.mjs` now classifies that as a blocked Rust harness launch and falls back to compile proof (`cargo test --no-run` in quick mode, `cargo check --lib` in the broader path) instead of reporting an undifferentiated failed step.
...
- Explorer live layout zoom intentionally decouples tile layout from icon-band sizing. `src/config/explorerViewModes.ts` now owns continuous layout metrics plus stepped icon metrics, `src/config/explorerTheme.ts` keeps `gridScale` / `spacingScale` on layout and `iconScale` on the stepped icon band, and `src/components/FileExplorer.tsx` pins the icon stage to the committed grid band during grid-to-grid wheel gestures. If zoom starts feeling laggy because icons grow every tick again, inspect that split before touching thumbnail caches, wheel sensitivity, or scrollbar math.
