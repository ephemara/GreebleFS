...
- `src/runtime/explorerViewportThumbnailScheduler.ts`
  First-class Explorer thumbnail scheduling runtime. It decides thumbnail read order and concurrency from the virtual viewport plus authored performance policy, with `hover`, `visible`, `forward-prefetch`, and `backward-prefetch` lanes. It must stay orchestration-only: artifact/model thumbnail generation remains in `explorerThumbnailArtifactRuntime.ts` and `modelThumbnailBackend.ts`.
