...
- Managed content roots now split by runtime mode around a canonical `usr/` backbone:
  ...
  - `usr/profiles/catalog.json`, `usr/profiles/shared/settings.json`, `usr/profiles/<profile-id>/{profile.json,settings.json}`, and the profile-owned lane folders under `usr/profiles/<profile-id>/<lane>` are the runtime-owned profile contract. `usr/profiles/default` is the canonical shipped baseline for workbench-config lanes, and startup migrates any legacy top-level overlay lanes into that default profile. V1 only moves workbench config behind profiles; notes, screenshots, explorer workspace/session blobs, and other transient local storage stay global outside this system.
