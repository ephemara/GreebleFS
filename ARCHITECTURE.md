...
- `src/runtime/pluginIndexApi.ts`
  Streamlined plugin-facing index adapter. Frontend package plugins should use `api.index.global`, `api.index.semantic`, and `api.index.media` here instead of raw invoke strings when they need indexed filesystem search, semantic RAG/search flows, or media discovery. `usr/plugins/greeblefs-index-photo-gallery` is the first-party smoke plugin that finds pictures from the native index and renders them without crawling the filesystem itself.
