...
- `src/components/pluginRuntime.tsx`
  Packaged frontend plugin runtime loader. It owns the allowlisted module graph for desktop frontend plugins, including package-local relative imports, `definePreviewLane(...)` preview-lane registration, `defineSettingsSlot(...)` plugin-settings registration, `defineWorkflow(...)` explorer-workflow registration, the host-provided `overlayterm-plugin` bridge helpers/runtime bridge, and the typed `api.index` plus `api.workflows` surfaces exposed to packaged React plugins.
- `src/config/pluginPackages.ts` and `src/runtime/useFolderPluginRuntime.ts`
  Packaged plugin discovery/aggregation lane. These files normalize package manifests, bind plugin preview-lane renderers plus plugin-authored settings-slot renderers and plugin-authored explorer workflows into the host runtime, preserve dual-surface metadata such as `contributions.mobilePanes`, inject the shared index API into every frontend plugin, aggregate diagnostics/capability counts, and surface the discovered `previewLanes`, `settingsSlots`, and `workflows` catalogs upward to `App.tsx`, the panel registry, Explorer, and Settings. `useFolderPluginRuntime.ts` also binds the explorer execution context into `api.workflows.open(...)` / `api.workflows.close()` so plugin code launches pane-targeted explorer workflows through the shared bridge rather than talking to `FileExplorer` state directly. Settings-backed enablement also enters here: `settings.plugins.enablementByPluginId` stores explicit disabled overrides, `folderPlugins` remains the full manager catalog, and `enabledFolderPlugins` is the shell-mountable subset used for panel/runtime registration.
