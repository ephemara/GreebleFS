...
- `MCP/greeblefs-dev-mcp/`
  Standalone dev automation subsystem for agents and MCP clients. `src/index.ts` is the public MCP server, `src/smoke.ts` is the runtime proof harness, `src/runtime/greeblefsAutomationRuntime.ts` owns attach/status/workspace automation, and `src/runtime/windowsDesktopWindowCapture.ts` is the Windows-native fallback for enumerating/capturing real desktop `greeblefs.exe` windows when WebView/CDP attach is unavailable. The package uses Node + `tsx` for runtime execution because Playwright/CDP attachment is reliable there on this Windows host, while Bun remains fine for install and TypeScript validation.
...
- On this Windows machine, `bun run tauri dev` currently launches the native app but the WebView2 remote-debugging port (`http://127.0.0.1:9222/json/version`) still refuses connections. The MCP runtime therefore falls back to `browser-dev-url` attachment plus Windows-native desktop-window enumeration/screenshot capture. If host bridge tools report `unavailable` or a browser screenshot is blank white, that means the fallback browser is not a real Tauri webview; use `app_list_native_windows` and `ui_native_window_screenshot` to inspect the actual desktop windows while debugging the native attach path.
