...
- Universal polyglot runtime pipeline (`runtime-host-v1`):
  ...
  - hardening contracts that future runtime work must preserve:
    ...
    - **Wasm artifact loading goes through the Tauri asset protocol.** `WasmPanelHost.tsx` resolves prepared Wasm artifacts through `convertFileSrc(...)`: Go/TinyGo fetch compiled `.wasm` bytes over that URL, while Rust `cargo-wasm-bindgen` imports the generated JS boot module from the same asset lane. Do not regress to raw `file://` fetches or direct disk imports.
