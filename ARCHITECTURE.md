...
- `src/runtime/explorerThumbnailArtifactRuntime.ts`
  Shared explorer thumbnail runtime above `FileExplorer.tsx`. It reads backend-owned thumbnail artifacts, converts absolute artifact paths into browser-safe local asset URLs, and caches results by `entityId + contentRevision + dimensions + hover variant` so folder opens and refreshes can preserve visible thumbnails instead of treating every relist as a cold start.
