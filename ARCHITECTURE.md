...
- `src-tauri/src/native_task_graph.rs`
  Rust-native Explorer task graph for scan-class background work. It is registered as Tauri state from `src-tauri/src/lib.rs`, loads the authored `nativeTaskGraph` policy from `usr/profiles/default/explorer-performance/greeblefs-core/explorer-performance.json`, and owns bounded queue pressure, stable priority/FIFO ordering, per-lane concurrency caps, stale work-key generation cancellation, parent/child cancellation metadata, cooperative cancellation tokens, typed `submit_blocking` / `submit_async` helpers, and timing/counter telemetry. New native Explorer background work should route through this manager instead of ad hoc `spawn_blocking` when it is user-visible, cancellable, cache-warming, scan-class, or likely to contend with directory/search/checksum work.
...
- `src/config/explorerPerformance.ts`
  Data-driven Explorer hot-path policy loader from `usr/profiles/default/explorer-performance/**/explorer-performance.json`. It owns folder-activation timing, navigation budgets, and viewport thumbnail scheduling policy; tune this manifest first before hardcoding batch sizes, settle delays, or prefetch spans in React.
