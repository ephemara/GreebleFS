...
- `src/config/explorerActivityRail.ts`, `src/components/explorer/ExplorerActivityRail.tsx`, `src/components/explorer/ExplorerDockLayoutAdapter.tsx`, `src/components/explorer/ExplorerSearchLane.tsx`, `src/components/explorer/ExplorerSemanticLane.tsx`, and `src/components/explorer/ExplorerPanePrimitives.tsx`
  Dense Explorer utility-pane shell. The activity rail is data-driven, split-capable, and persists the active left lane/utility-pane visibility through `explorerStore` session snapshots. `explorerActivityRailDefinitionsBySide` owns left/right rail placement: Files/Search/Semantic/Tasks/Terminal stay on the left rail while Preview/Actions/Customize mount on the right rail and can coexist as a split right activity dock without stealing the left dock selection. `ExplorerDockLayoutAdapter` is the only `flexlayout-react` integration point and keeps the main file grid/list under GreebleFS ownership while Search, Semantic, Tasks, and future utility lanes can dock/tab independently. Search uses Rust-backed `explorerBackend` requests for real filesystem name/content scans and uses `fuse.js` only for local filtering inside already-loaded result groups. Semantic indexing/search stays on the existing semantic backend; the lane is an Explorer affordance for index/rebuild/status/find-similar rather than a second indexing engine. Reuse the slate pane primitives for new Explorer utility panes instead of adding ad hoc card chrome.
