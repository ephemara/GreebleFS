...
<!-- Updated: 2026-04-18T06:45:31.604Z -->
...
```
src/App.tsx                                   ~App
src/components/DevPerformanceHud.tsx          ~DevPerformanceHud
src/components/ExplorerArchivePreview.tsx     +ExplorerArchivePreview  +formatSize
src/components/ExplorerImageEditor.tsx        +isEditableKeyboardTarget  +buttonStyle  +PremiumSlider  ~cloneEditorState
src/components/ExplorerAudioWorkbench.tsx     +syncPlayhead  +updateSelection  +updateFadeIn  +updateFadeOut
src/components/ExplorerVideoEditor.tsx        +clamp  +formatTimecode  +audioBufferToWav  +ControlSlider
src/components/FileExplorer.tsx               ~PreviewPanel  ~FileExplorer
src/components/SettingsPage.tsx               ~SettingsPage  ~ThemeBadge  ~SettingsRailButton
src/components/TerminalOverlay.tsx            +getTerminalPaneDomKey  +applyTerminalPaneFrameToElement  +applyTerminalSplitHandleToElement  +shouldKeepTerminalViewportPinnedToBottom
src/config/imageEditorFilters.ts              +createDefaultImageFiltersState  +buildCSSFilterString
src/runtime/audioWorkbenchBackend.ts          +loadExplorerAudioDeckPlugin  +clearExplorerAudioDeckPlugin  ~listenToExplorerAudioEngineState
src/runtime/explorerBackend.ts                +inspectExplorerArchive  ~openExplorerArchive  ~openExplorerPathWithDialog
src/runtime/tauriClient.ts                    +summarizeArgs  +wrapCommand  +wrapEvents  ~unwrapTauriResult
src/runtime/imageEditorBackend.ts             +createExplorerImageEditorSession  +renderExplorerImageEditorPreview  +exportExplorerImageEditorResult  +closeExplorerImageEditorSession
src/runtime/telemetry.ts                      +getPerformanceNow  +getRecordedAt  +telemetryEnabled  +sourceTraceEnabled
src/runtime/telemetryBackend.ts               +buildTelemetryConfigFromSettings  +configureTelemetry  +getTelemetryStatus  +getRecentTelemetryRecords
src/runtime/vstBackend.ts                     +getExplorerVstDefaultScanPaths  +scanExplorerVstPlugins
src/store/audioEngineStore.ts                 +loadAudioDeckPlugin  +clearAudioDeckPlugin  ~setAudioDeckRate
src/store/settingsStore.ts                    +normalizeDeveloperTelemetryCaptureMode  +normalizeDeveloperTelemetryPayloadMode  +normalizeTelemetryFileSizeMb  +normalizeAudioSettings
crates/ffmpeg-suite-rs/rust_ffmpeg/src/builder.rs +pointer
```
...
### crates/ffmpeg-suite-rs/.idea/vcs.xml
```
root project
component[name=VcsDirectoryMappings]
```

### crates/ffmpeg-suite-rs/.idea/modules.xml
```
root project
...
### crates/ffmpeg-suite-rs/README.md
```
h1 FFmpeg Suite Rust Wrappers
h2 Overview
h2 Features
h2 Installation
h2 Quick Start
h3 FFmpeg - Transcoding
h3 FFprobe - Media Inspection
h3 FFplay - Media Playback
...
### crates/ffmpeg-suite-rs/Cargo.toml
```
table [workspace]
...
### crates/ffmpeg-suite-rs/rust_ffplay/src/playback.rs
```
...
### crates/ffmpeg-suite-rs/rust_ffplay/src/display.rs
```
...
### crates/ffmpeg-suite-rs/rust_ffplay/src/builder.rs
```
...
impl FFplayBuilder
impl FFplayProcess
  pub async fn wait(self) → Result<std::process::ExitSt...
  pub async fn kill(&mut self) → Result<()>
  pub fn id(&self) → Option<u32>
  pub fn try_wait(&mut self) → Result<Option<std::process:...
impl FFplayBuilder
...
### crates/ffmpeg-suite-rs/rust_ffplay/src/types.rs
```
