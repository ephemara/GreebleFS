...
impl<R: Runtime> WebviewManager<R> {
  ...
  fn prepare_pending_webview<M: Manager<R>>(
    ...
    if !registered_scheme_protocols.contains(&"transport".into()) {
      let protocol = crate::transport::protocol(manager.manager_owned(), window_origin.clone());
      pending.register_uri_scheme_protocol("transport", move |webview_id, request, responder| {
        protocol(webview_id, request, UriSchemeResponder(responder))
      });
    }
