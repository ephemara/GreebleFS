...
#[cfg(windows)]
pub use tauri_runtime::webview::WindowsWebview2VisualHostingMode;
...
impl PlatformWebview {
  ...
  /// Returns the WebView2 composition controller when the webview uses DirectComposition visual hosting.
  #[cfg(windows)]
  #[cfg_attr(docsrs, doc(cfg(windows)))]
  pub fn composition_controller(&self) -> Option<*mut c_void> {
    self.0.composition_controller.clone()
  }

  /// Returns the DirectComposition device used by the WebView2 visual host.
  #[cfg(windows)]
  #[cfg_attr(docsrs, doc(cfg(windows)))]
  pub fn composition_device(&self) -> Option<*mut c_void> {
    self.0.composition_device.clone()
  }

  /// Returns the DirectComposition target used by the WebView2 visual host.
  #[cfg(windows)]
  #[cfg_attr(docsrs, doc(cfg(windows)))]
  pub fn composition_target(&self) -> Option<*mut c_void> {
    self.0.composition_target.clone()
  }

  /// Returns the root visual that owns the WebView2 composition subtree.
  #[cfg(windows)]
  #[cfg_attr(docsrs, doc(cfg(windows)))]
  pub fn composition_root_visual(&self) -> Option<*mut c_void> {
    self.0.composition_root_visual.clone()
  }

  /// Returns the WebView2 visual that receives DOM content in composition-controller mode.
  #[cfg(windows)]
  #[cfg_attr(docsrs, doc(cfg(windows)))]
  pub fn composition_webview_visual(&self) -> Option<*mut c_void> {
    self.0.composition_webview_visual.clone()
  }

  /// Whether this webview uses WebView2's DirectComposition composition controller.
  #[cfg(windows)]
  #[cfg_attr(docsrs, doc(cfg(windows)))]
  pub fn is_webview2_composition_controller(&self) -> bool {
    self.0.composition_controller.is_some()
  }
...
```
  ...
  /// Selects how WebView2 is hosted in the native Windows visual tree.
  ///
  /// Defaults to [`WindowsWebview2VisualHostingMode::Hwnd`]. Use
  /// [`WindowsWebview2VisualHostingMode::CompositionController`] to attach WebView2 through its
  /// DirectComposition composition controller.
  #[cfg(windows)]
  #[must_use]
  pub fn windows_webview2_visual_hosting_mode(
    mut self,
    mode: WindowsWebview2VisualHostingMode,
  ) -> Self {
    self.webview_attributes = self
      .webview_attributes
      .windows_webview2_visual_hosting_mode(mode);
    self
  }
