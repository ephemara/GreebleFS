...
use std::{
  borrow::Cow, cell::RefCell, collections::HashSet, ffi::c_void, fmt::Write, fs, path::PathBuf,
  rc::Rc,
  sync::mpsc,
};
...
const COMPOSITION_CONTROLLER_SUBCLASS_ID: u32 = WM_USER + 0x67;
const COMPOSITION_CONTROLLER_DESTROY_MESSAGE: u32 = WM_USER + 0x68;
...
#[allow(dead_code)]
struct WebView2CompositionHost {
...
struct CompositionControllerSubclassState {
...
pub(crate) struct InnerWebView {
  ...
  composition_host: Option<WebView2CompositionHost>,
...
impl Drop for InnerWebView {
  fn drop(&mut self) {
    ...
    unsafe { Self::dettach_composition_controller_subclass(self.hwnd) };
    if self.is_child {
...
impl WebView2CompositionHost {
  fn new(hwnd: HWND, composition_controller: ICoreWebView2CompositionController) -> Result<Self> {
    unsafe {
      ...
      root_visual.AddVisual(&webview_visual, false, None)?;
      target.SetRoot(&root_visual)?;
      composition_controller.SetRootVisualTarget(&webview_visual)?;
      device.Commit()?;
...
impl InnerWebView {
  ...
  fn new_in_hwnd(
    ...
    let (controller, composition_host) = if pl_attrs.webview2_composition_controller {
      ...
      let composition_host = WebView2CompositionHost::new(hwnd, composition_controller)?;
      (controller, Some(composition_host))
    } else {
      (
    ...
    if let Some(composition_host) = &composition_host {
      unsafe {
        Self::attach_composition_controller_subclass(
          hwnd,
          &controller,
          &composition_host.composition_controller,
        )
      };
    }
  ...
  unsafe extern "system" fn composition_controller_subclass_proc(
    hwnd: HWND,
    msg: u32,
    wparam: WPARAM,
    lparam: LPARAM,
    ...
    let state = dwrefdata as *mut CompositionControllerSubclassState;
    ...
    match msg {
      WM_SIZE => {
        if wparam.0 != SIZE_MINIMIZED as usize {
          if let Ok(PhysicalSize { width, height }) = Self::parent_bounds(hwnd) {
      ...
      WM_LBUTTONDOWN | WM_RBUTTONDOWN | WM_MBUTTONDOWN | WM_XBUTTONDOWN | WM_LBUTTONDBLCLK
      | WM_RBUTTONDBLCLK | WM_MBUTTONDBLCLK | WM_XBUTTONDBLCLK => {
        let _ = SetCapture(hwnd);
        let _ = SetFocus(Some(hwnd));
        Self::send_composition_mouse_input(hwnd, msg, wparam, lparam, state);
        return LRESULT(0);
      }
      ...
      msg if msg == WM_DESTROY || msg == COMPOSITION_CONTROLLER_DESTROY_MESSAGE => {
        drop(Box::from_raw(state));
        let _ = SetWindowSubclass(
          hwnd,
          Some(Self::composition_controller_subclass_proc),
          COMPOSITION_CONTROLLER_SUBCLASS_ID as _,
          std::ptr::null::<()>() as _,
        );
      }
  ...
  #[inline]
  unsafe fn attach_composition_controller_subclass(
    hwnd: HWND,
    controller: &ICoreWebView2Controller,
    composition_controller: &ICoreWebView2CompositionController,
  ) {
    let state = CompositionControllerSubclassState {
  ...
  #[inline]
  unsafe fn dettach_composition_controller_subclass(hwnd: HWND) {
    SendMessageW(hwnd, COMPOSITION_CONTROLLER_DESTROY_MESSAGE, None, None);
    let _ = RemoveWindowSubclass(
      hwnd,
      Some(Self::composition_controller_subclass_proc),
      COMPOSITION_CONTROLLER_SUBCLASS_ID as _,
  ...
  #[inline]
  unsafe fn send_composition_mouse_input(
    hwnd: HWND,
    message: u32,
    wparam: WPARAM,
    lparam: LPARAM,
    state: *mut CompositionControllerSubclassState,
  ) {
    let Some(event_kind) = Self::composition_mouse_event_kind(message) else {
      return;
    };
    let mut point = POINT {
      x: Self::low_word_signed(lparam.0),
      y: Self::high_word_signed(lparam.0),
    };
    let mouse_data = match message {
      WM_MOUSEWHEEL | WM_MOUSEHWHEEL => {
        let _ = ScreenToClient(hwnd, &mut point);
        Self::high_word_signed(wparam.0 as isize) as u32
      }
    ...
    let _ = (*state).composition_controller.SendMouseInput(
      event_kind,
  ...
  #[inline]
  fn composition_mouse_event_kind(message: u32) -> Option<COREWEBVIEW2_MOUSE_EVENT_KIND> {
    match message {
      ...
      WM_MBUTTONDBLCLK => Some(COREWEBVIEW2_MOUSE_EVENT_KIND_MIDDLE_BUTTON_DOUBLE_CLICK),
      WM_XBUTTONDOWN => Some(COREWEBVIEW2_MOUSE_EVENT_KIND_X_BUTTON_DOWN),
      WM_XBUTTONUP => Some(COREWEBVIEW2_MOUSE_EVENT_KIND_X_BUTTON_UP),
      WM_XBUTTONDBLCLK => Some(COREWEBVIEW2_MOUSE_EVENT_KIND_X_BUTTON_DOUBLE_CLICK),
      _ => None,
    }
  }
...
impl InnerWebView {
  ...
  pub fn composition_device(&self) -> Option<*mut c_void> {
    self.composition_host
      .as_ref()
      .map(|host| windows::core::Interface::as_raw(&host.device) as *mut c_void)
  }

  pub fn composition_target(&self) -> Option<*mut c_void> {
    self.composition_host
      .as_ref()
      .map(|host| windows::core::Interface::as_raw(&host.target) as *mut c_void)
  }

  pub fn composition_root_visual(&self) -> Option<*mut c_void> {
    self.composition_host
      .as_ref()
      .map(|host| windows::core::Interface::as_raw(&host.root_visual) as *mut c_void)
  }

  pub fn composition_webview_visual(&self) -> Option<*mut c_void> {
    self
      .composition_host
      .as_ref()
      .map(|host| windows::core::Interface::as_raw(&host.webview_visual) as *mut c_void)
  }

  pub fn composition_controller(&self) -> Option<*mut c_void> {
    self
      .composition_host
      .as_ref()
      .map(|host| windows::core::Interface::as_raw(&host.composition_controller) as *mut c_void)
  }

  pub fn is_webview2_composition_controller(&self) -> bool {
    self.composition_host.is_some()
  }
