...
pub(crate) struct PlatformSpecificWebViewAttributes {
  ...
  webview2_composition_controller: bool,
}
...
impl Default for PlatformSpecificWebViewAttributes {
  fn default() -> Self {
    Self {
      ...
      extension_path: None,
      environment: None,
      webview2_composition_controller: false,
    }
  }
}
...
pub trait WebViewBuilderExtWindows {
  ...
  /// Uses WebView2's DirectComposition-backed composition controller instead of the default HWND controller.
  ///
  /// This is only a visual hosting mode switch: WebView2 still owns DOM, layout and rasterization,
  /// while native code can compose the resulting visual with other DirectComposition surfaces.
  fn with_webview2_composition_controller(self, enabled: bool) -> Self;
}
...
impl WebViewBuilderExtWindows for WebViewBuilder<'_> {
  ...
  fn with_webview2_composition_controller(mut self, enabled: bool) -> Self {
    self.platform_specific.webview2_composition_controller = enabled;
    self
  }
}
...
pub trait WebViewExtWindows {
  ...
  /// WebView2 composition controller when this view was created in DirectComposition visual hosting mode.
  fn composition_controller(&self) -> Option<*mut std::ffi::c_void>;

  /// DirectComposition device used by the WebView2 visual host when composition hosting is enabled.
  fn composition_device(&self) -> Option<*mut std::ffi::c_void>;

  /// DirectComposition target used by the WebView2 visual host when composition hosting is enabled.
  fn composition_target(&self) -> Option<*mut std::ffi::c_void>;

  /// The root visual that owns the WebView2 composition subtree.
  fn composition_root_visual(&self) -> Option<*mut std::ffi::c_void>;

  /// The WebView2 visual that receives DOM content in composition-controller mode.
  fn composition_webview_visual(&self) -> Option<*mut std::ffi::c_void>;

  /// Whether this view is hosted by WebView2's composition controller instead of the default HWND controller.
  fn is_webview2_composition_controller(&self) -> bool;
...
impl WebViewExtWindows for WebView {
  ...
  fn composition_controller(&self) -> Option<*mut std::ffi::c_void> {
    self.webview.composition_controller()
  }

  fn composition_device(&self) -> Option<*mut std::ffi::c_void> {
    self.webview.composition_device()
  }

  fn composition_target(&self) -> Option<*mut std::ffi::c_void> {
    self.webview.composition_target()
  }

  fn composition_root_visual(&self) -> Option<*mut std::ffi::c_void> {
    self.webview.composition_root_visual()
  }

  fn composition_webview_visual(&self) -> Option<*mut std::ffi::c_void> {
    self.webview.composition_webview_visual()
  }

  fn is_webview2_composition_controller(&self) -> bool {
    self.webview.is_webview2_composition_controller()
  }
