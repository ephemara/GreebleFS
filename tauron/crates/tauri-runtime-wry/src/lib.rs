...
impl Drop for WebviewWrapper {
  fn drop(&mut self) {
    if Rc::get_mut(&mut self.inner).is_some() {
      #[cfg(windows)]
      windows_webview2_devtools::drop_webview_diagnostics_record(&self.label);
...
impl<T: UserEvent> Wry<T> {
  ...
  fn init(event_loop: EventLoop<Message<T>>) -> Result<Self> {
    ...
    #[cfg(windows)]
    {
      let webview_runtime_version = wry::webview_version().ok();
      windows_webview2_devtools::publish_runtime_bootstrap(
        context.webview_runtime_installed,
        webview_runtime_version.as_deref(),
      );
    }
          ...
          WebviewMessage::WithWebview(f) => {
            ...
            #[cfg(windows)]
            {
              f(Webview {
                ...
                composition_device: webview.composition_device(),
                composition_target: webview.composition_target(),
                composition_root_visual: webview.composition_root_visual(),
                composition_webview_visual: webview.composition_webview_visual(),
                composition_controller: webview.composition_controller(),
              });
            }
            #[cfg(target_os = "android")]
            {
...
"#,
  ...
  let reused_existing_web_context;
  let web_context = match entry {
    Occupied(occupied) => {
      reused_existing_web_context = true;
      let occupied = occupied.into_mut();
      occupied.referenced_by_webviews.insert(label.clone());
      occupied
    }
    Vacant(vacant) => {
      reused_existing_web_context = false;
  ...
  #[cfg(any(debug_assertions, feature = "devtools"))]
  let resolved_devtools_enabled = webview_attributes.devtools.unwrap_or(true);
  #[cfg(not(any(debug_assertions, feature = "devtools")))]
  let resolved_devtools_enabled = false;

  #[cfg(windows)]
  let windows_webview2_visual_hosting_mode =
    webview_attributes.windows_webview2_visual_hosting_mode;

  #[cfg(windows)]
  let windows_webview2_devtools_capture =
    windows_webview2_devtools::WindowsWebview2DevtoolsCapture::new(
      &label,
      kind,
      windows_webview2_visual_hosting_mode,
      &url,
      web_context_key.as_deref(),
      webview_attributes.additional_browser_args.as_deref(),
      webview_attributes.browser_extensions_enabled,
      webview_attributes.scroll_bar_style,
      resolved_devtools_enabled,
      webview_attributes.use_https_scheme,
      webview_attributes.incognito,
      webview_attributes.transparent,
      webview_attributes.focus,
      automation_enabled,
      automation_enabled && is_first_context,
      webview_attributes.environment.is_some(),
      reused_existing_web_context,
      window.theme(),
    );
  #[cfg(windows)]
  windows_webview2_devtools_capture.publish_create_attempt();
  ...
  #[cfg(windows)]
  {
    if let Some(additional_browser_args) =
      windows_webview2_devtools_capture.resolved_additional_browser_args()
    {
    ...
    webview_builder = webview_builder.with_webview2_composition_controller(matches!(
      windows_webview2_visual_hosting_mode,
      WindowsWebview2VisualHostingMode::CompositionController
    ));
  }
  ...
  #[cfg(any(debug_assertions, feature = "devtools"))]
  {
    webview_builder = webview_builder.with_devtools(resolved_devtools_enabled);
  }
  ...
  if let Err(error) = &webview_result {
    windows_webview2_devtools_capture.publish_create_failure(&error.to_string());
  }
  let webview = webview_result.map_err(|e| Error::CreateWebview(Box::new(e)))?;
  ...
  #[cfg(windows)]
  windows_webview2_devtools_capture
    .publish_create_success(id, &format!("0x{:X}", window.hwnd() as usize));
